...
/* $Id: update.php,v 1.1 2006/03/27 08:39:35 mikhail Exp $ */
...
if (!$read_me) {
    ...
    if ($readme_file) {
        ...
        print nl2br(htmlentities(implode(' ', $readme), ENT_QUOTES | ENT_HTML5));
...
if (!$filename) {
    ...
    while ($f = $files->read()) {
        if ('.sql' == mb_substr($f, -4, 4)) {
            print "<LI><A HREF=\"update.php?read_me=1&filename=$f\">$f</A>";

            $sql_file = file($f);

            if ('# Brief:' == mb_substr($sql_file[0], 0, 8)) {
                print ' - ' . htmlentities(mb_substr($sql_file[0], 9), ENT_QUOTES | ENT_HTML5);
            }
    ...
    if (!$action) {
        ...
        foreach ($sql_file as $k => $v) {
            ...
            if (';' == mb_substr($sql, -1, 1)) {
                $sql_statements[] = mb_substr(trim($cur_sql), 0, -1);
        ...
        foreach ($sql_statements as $k => $v) {
            print '<B>' . $k . ":</B> <TEXTAREA NAME=\"sql_statements[$k]\" ROWS=\"3\" COLS=\"40\" WRAP=\"soft\">" . htmlentities($v, ENT_QUOTES | ENT_HTML5) . "</TEXTAREA><br><br>\n";
        }
        ...
        foreach ($sql_statements as $k => $v) {
            $result = $GLOBALS['xoopsDB']->queryF(stripslashes($v));

            if (!$result) {
                print "<B>Error:</B> the following SQL statement didn't work.<br>\n";

                print $GLOBALS['xoopsDB']->error() . '<br><br>';

                print "<BLOCKQUOTE>\n" . htmlentities(stripslashes($v), ENT_QUOTES | ENT_HTML5) . "\n</BLOCKQUOTE>";
