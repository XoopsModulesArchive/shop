...
/*
  $Id: ms1_to_ms2.php,v 1.1 2006/03/27 08:39:37 mikhail Exp $
...
function osc_db_query($db_query)
{
    ...
    $result = $GLOBALS['xoopsDB']->queryF($db_query, $db_link) or osc_db_error($db_query, $GLOBALS['xoopsDB']->errno(), $GLOBALS['xoopsDB']->error());
...
osc_db_connect() || die('Unable to connect to database server!');
?>
...
function osc_db_update_configuration_key($key, $new_key)
{
    ...
    $db_error = $GLOBALS['xoopsDB']->error();
...
$ab_id = 1;
while (false !== ($ab = osc_db_fetch_array($ab_query))) {
...
if (osc_db_num_rows($country_query) > 0) {
    ...
    if ('' != $country['name']) {
        ...
        if (null != $new_country['countries_iso_code_2']) {
