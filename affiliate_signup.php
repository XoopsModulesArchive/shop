...
/*
  $Id: affiliate_signup.php,v 2.00 2004/10/12
...
if (isset($_POST['action'])) {
    ...
    if (mb_strlen($a_firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
        $error = true;
    ...
    if (mb_strlen($a_lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
        $error = true;
    ...
    if (ACCOUNT_DOB == 'true') {
        if (checkdate(mb_substr(tep_date_raw($a_dob), 4, 2), mb_substr(tep_date_raw($a_dob), 6, 2), mb_substr(tep_date_raw($a_dob), 0, 4))) {
    ...
    if (mb_strlen($a_email_address) < ENTRY_EMAIL_ADDRESS_MIN_LENGTH) {
        $error = true;
    ...
    if (mb_strlen($a_street_address) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
        $error = true;
    ...
    if (mb_strlen($a_postcode) < ENTRY_POSTCODE_MIN_LENGTH) {
        $error = true;
    ...
    if (mb_strlen($a_city) < ENTRY_CITY_MIN_LENGTH) {
        $error = true;
    ...
    if (mb_strlen($a_telephone) < ENTRY_TELEPHONE_MIN_LENGTH) {
        $error = true;
...
'affiliate_newsletter' => $a_newsletter,
        ...
        $aemailbody = MAIL_AFFILIATE_HEADER
                      ...
                      . $affiliate_id
                      . "\n"
                      . MAIL_AFFILIATE_USERNAME
                      . $a_email_address
                      . "\n"
                      . MAIL_AFFILIATE_PASSWORD
                      . $a_password
                      . "\n\n"
                      . MAIL_AFFILIATE_LINK
                      . HTTP_SERVER
...
<table border="0" width="100%" cellspacing="3" cellpadding="3">
    <tr>
        ...
        <!-- body_text //-->
        <td width="100%" valign="top"><?php echo tep_draw_form('affiliate_signup', tep_href_link(FILENAME_AFFILIATE_SIGNUP, '', 'SSL'), 'post') . tep_draw_hidden_field('action', 'process'); ?>
                ...
                <tr>
                    <td>
                        ...
                        if (isset($_GET['affiliate_email_address'])) {
                            $a_email_address = tep_db_prepare_input($_GET['affiliate_email_address']);
                        }
