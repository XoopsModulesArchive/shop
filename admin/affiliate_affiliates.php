...
/*
  $Id: affiliate_affiliates.php,v 2.00 2004/10/12
...
'affiliate_agb' => '1',
            ...
            if (ACCOUNT_DOB == 'true') {
                $sql_data_array['affiliate_dob'] = tep_date_raw($affiliate_dob);
            }
            ...
            tep_redirect(tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(['acID', 'action']) . 'acID=' . $affiliate_id));
            break;
        case 'deleteconfirm':
            $affiliate_id = tep_db_prepare_input($_GET['acID']);
            ...
            tep_redirect(tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(['acID', 'action'])));
            break;
    }
}
?>
...
<head>
    ...
    if ('edit' == $_GET['action']) {
        ...
        <script language="javascript"><!--
            ...
            echo 'var affiliate_company = document.affiliate.affiliate_company.value;' . "\n";
        } ?>
        ...
        <td width="100%" valign="top">
            <table border="0" width="100%" cellspacing="0" cellpadding="2">
                ...
                if ('edit' == $_GET['action']) {
                    ...
                    <tr><?php echo tep_draw_form('affiliate', FILENAME_AFFILIATE, tep_get_all_get_params(['action']) . 'action=update', 'post', 'onSubmit="return check_form();"'); ?>
                            ...
                            <table border="0" cellspacing="2" cellpadding="2">
                                ...
                                if (ACCOUNT_GENDER == 'true') {
                                    ...
                                    <tr>
                                        ...
                                        <td class="main"><?php echo tep_draw_radio_field('affiliate_gender', 'm', false, $aInfo->affiliate_gender)
                                                                    . '&nbsp;&nbsp;'
                                                                    . MALE
                                                                    . '&nbsp;&nbsp;'
                                                                    . tep_draw_radio_field('affiliate_gender', 'f', false, $aInfo->affiliate_gender)
                                                                    . '&nbsp;&nbsp;'
                                                                    . FEMALE; ?></td>
                                    </tr>
                                    <?php
                    ...
                    <tr>
                        <td align="right" class="main"><?php echo tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(['action'])) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?></td>
                    </tr></form>
                    <?php
                } else {
                    ?>
                    ...
                    $affiliate_split = new splitPageResults(
                        $_GET['page'],
                        MAX_DISPLAY_SEARCH_RESULTS,
                        $affiliate_query_raw,
                        $affiliate_query_numrows
                    );

                    $affiliate_query = tep_db_query($affiliate_query_raw);

                    while (false !== ($affiliate = tep_db_fetch_array($affiliate_query))) {
                        $info_query = tep_db_query(
                            'select affiliate_commission_percent, affiliate_date_account_created as date_account_created, affiliate_date_account_last_modified as date_account_last_modified, affiliate_date_of_last_logon as date_last_logon, affiliate_number_of_logons as number_of_logons from '
                                                    . TABLE_AFFILIATE
                                                    . " where affiliate_id = '"
                        ...
                        if ((is_object($aInfo)) && ($affiliate['affiliate_id'] == $aInfo->affiliate_id)) {
                            echo '          <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . tep_href_link(
                                FILENAME_AFFILIATE,
                                tep_get_all_get_params(['acID', 'action']) . 'acID=' . $aInfo->affiliate_id . '&action=edit'
                            ) . '\'">' . "\n";
                        } else {
                            echo '          <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(
                                FILENAME_AFFILIATE,
                                tep_get_all_get_params(['acID']) . 'acID=' . $affiliate['affiliate_id']
                            ) . '\'">' . "\n";
                        }

                        if ('http://' != mb_substr($affiliate['affiliate_homepage'], 0, 7)) {
                            $affiliate['affiliate_homepage'] = 'http://' . $affiliate['affiliate_homepage'];
                        } ?>
                                                ...
                                                <td class="dataTableContent" align="right"><?php if ($affiliate['affiliate_commission_percent'] > AFFILIATE_PERCENT) {
                            echo $affiliate['affiliate_commission_percent'];
                        } else {
                            echo AFFILIATE_PERCENT;
                        } ?> %
                                                </td>
                                                <td class="dataTableContent"><?php echo '<a href="'
                                                                                        . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(['acID', 'action']) . 'acID=' . $affiliate['affiliate_id'] . '&action=edit')
                                                                                        . '">'
                                                                                        . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW)
                                                                                        . '</a>';

                        echo '<a href="' . $affiliate['affiliate_homepage'] . '" target="_blank">' . $affiliate['affiliate_homepage'] . '</a>'; ?></td>
                                                <td class="dataTableContent" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_STATISTICS, tep_get_all_get_params(['acID']) . 'acID=' . $affiliate['affiliate_id']) . '">' . tep_image(
                            DIR_WS_ICONS . 'statistics.gif',
                            ICON_STATISTICS
                        ) . '</a>&nbsp;';
                        ...
                        if ((is_object($aInfo)) && ($affiliate['affiliate_id'] == $aInfo->affiliate_id)) {
                            ...
                            echo '<a href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(['acID']) . 'acID=' . $affiliate['affiliate_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
                        } ?>&nbsp;
                                                </td>
                                                </tr>
                                                <?php
                                                ...
                                                <td colspan="6">
                                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                                        <tr>
                                                            ...
                                                            <td class="smallText" align="right"><?php echo $affiliate_split->display_links(
                            $affiliate_query_numrows,
                            MAX_DISPLAY_SEARCH_RESULTS,
                            MAX_DISPLAY_PAGE_LINKS,
                            $_GET['page'],
                            tep_get_all_get_params(['page', 'info', 'x', 'y', 'acID'])
                        ); ?></td>
                                                        </tr>
                                                        <?php
                    ...
                    switch ($_GET['action']) {
                                        case 'confirm':
                                            ...
                                            $contents = ['form' => tep_draw_form('affiliate', FILENAME_AFFILIATE, tep_get_all_get_params(['acID', 'action']) . 'acID=' . $aInfo->affiliate_id . '&action=deleteconfirm')];
                                            $contents[] = ['text' => TEXT_DELETE_INTRO . '<br><br><b>' . $aInfo->affiliate_firstname . ' ' . $aInfo->affiliate_lastname . '</b>'];
                                            $contents[] = [
                                                'align' => 'center',
'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(['acID', 'action']) . 'acID=' . $aInfo->affiliate_id) . '">' . tep_image_button(
    'button_cancel.gif',
    IMAGE_CANCEL
) . '</a>',
                                            ];
                                        ...
                                        default:
                                            if (is_object($aInfo)) {
                                                $heading[] = ['text' => '<b>' . $aInfo->affiliate_firstname . ' ' . $aInfo->affiliate_lastname . '</b>'];

                                                $contents[] = [
                                                    'align' => 'center',
'text' => '<a href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(['acID', 'action']) . 'acID=' . $aInfo->affiliate_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(
    FILENAME_AFFILIATE,
    tep_get_all_get_params(['acID', 'action']) . 'acID=' . $aInfo->affiliate_id . '&action=confirm'
) . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a> <a href="' . tep_href_link(FILENAME_AFFILIATE_CONTACT, 'selected_box=affiliate&affiliate=' . $aInfo->affiliate_email_address) . '">' . tep_image_button(
                                                            'button_email.gif',
                                                            IMAGE_EMAIL
                                                        ) . '</a>',
                                                ];
                                                ...
                                                $affiliate_sales_raw = 'select count(*) as count, sum(affiliate_value) as total, sum(affiliate_payment) as payment from '
                                                                          ...
                                                                          . ' a left join '
                                                                          . TABLE_ORDERS
                                                                          . ' o on (a.affiliate_orders_id=o.orders_id) where o.orders_status >= '
                                                ...
                                                $contents[] = ['text' => '<br>' . TEXT_DATE_ACCOUNT_CREATED . ' ' . tep_date_short($aInfo->date_account_created)];

                                                $contents[] = ['text' => '' . TEXT_DATE_ACCOUNT_LAST_MODIFIED . ' ' . tep_date_short($aInfo->date_account_last_modified)];

                                                $contents[] = ['text' => '' . TEXT_INFO_DATE_LAST_LOGON . ' ' . tep_date_short($aInfo->date_last_logon)];

                                                $contents[] = ['text' => '' . TEXT_INFO_NUMBER_OF_LOGONS . ' ' . $aInfo->number_of_logons];

                                                $contents[] = ['text' => '' . TEXT_INFO_COMMISSION . ' ' . $aInfo->affiliate_commission_percent . ' %'];

                                                $contents[] = ['text' => '' . TEXT_INFO_COUNTRY . ' ' . $aInfo->countries_name];

                                                $contents[] = ['text' => '' . TEXT_INFO_NUMBER_OF_SALES . ' ' . $affiliate_sales['count'], ''];

                                                $contents[] = ['text' => '' . TEXT_INFO_SALES_TOTAL . ' ' . $currencies->display_price($affiliate_sales['total'], '')];

                                                $contents[] = ['text' => '' . TEXT_INFO_AFFILIATE_TOTAL . ' ' . $currencies->display_price($affiliate_sales['payment'], '')];
                                            }
                                            break;
                                    }
