...
/*
  $Id: html_graphs.php,v 1.1 2006/03/27 09:06:04 mikhail Exp $
...
function start_graph($vals, $names)
{
    ...
    if (($vals['vlabel']) || ($vals['hlabel'])) {
        if ((0 == $vals['type']) || (2 == $vals['type'])) {
            // horizontal chart

            $rowspan = count($names) + 1;

            $colspan = 3;
        } elseif ((1 == $vals['type']) || (3 == $vals['type'])) {
            // vertical chart
...
function tep_banner_graph_infoBox($banner_id, $days)
{
    ...
    $banner_stats_query = tep_db_query(
        'select dayofmonth(banners_history_date) as name, banners_shown as value, banners_clicked as dvalue from ' . TABLE_BANNERS_HISTORY . " where banners_id = '" . $banner_id . "' and to_days(now()) - to_days(banners_history_date) < " . $days . ' order by banners_history_date'
    );

    while (false !== ($banner_stats = tep_db_fetch_array($banner_stats_query))) {
...
function tep_banner_graph_yearly($banner_id)
{
    ...
    while (false !== ($banner_stats = tep_db_fetch_array($banner_stats_query))) {
        ...
        $values[] = (($banner_stats['value']) ?: '0');

        $dvalues[] = (($banner_stats['dvalue']) ?: '0');
    }
...
function tep_banner_graph_monthly($banner_id)
{
    ...
    $banner_stats_query = tep_db_query(
        'select month(banners_history_date) as banner_month, sum(banners_shown) as value, sum(banners_clicked) as dvalue from ' . TABLE_BANNERS_HISTORY . " where banners_id = '" . $banner_id . "' and year(banners_history_date) = '" . $year . "' group by month(banners_history_date)"
    );

    while (false !== ($banner_stats = tep_db_fetch_array($banner_stats_query))) {
        $names[($banner_stats['banner_month'] - 1)] = strftime('%b', mktime(0, 0, 0, $banner_stats['banner_month']));

        $values[($banner_stats['banner_month'] - 1)] = (($banner_stats['value']) ?: '0');

        $dvalues[($banner_stats['banner_month'] - 1)] = (($banner_stats['dvalue']) ?: '0');
    }
...
function tep_banner_graph_daily($banner_id)
{
    ...
    $banner_stats_query = tep_db_query(
        'select dayofmonth(banners_history_date) as banner_day, banners_shown as value, banners_clicked as dvalue from ' . TABLE_BANNERS_HISTORY . " where banners_id = '" . $banner_id . "' and month(banners_history_date) = '" . $month . "' and year(banners_history_date) = '" . $year . "'"
    );

    while (false !== ($banner_stats = tep_db_fetch_array($banner_stats_query))) {
        $names[($banner_stats['banner_day'] - 1)] = $banner_stats['banner_day'];

        $values[($banner_stats['banner_day'] - 1)] = (($banner_stats['value']) ?: '0');

        $dvalues[($banner_stats['banner_day'] - 1)] = (($banner_stats['dvalue']) ?: '0');
    }
