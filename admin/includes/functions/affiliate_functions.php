...
/*
  $Id: affiliate_functions.php,v 2.00 2004/10/12
...
function affiliate_delete($affiliate_id)
{
    ...
    if ($affiliate = tep_db_fetch_array($affiliate_query)) {
        if ($affiliate['affiliate_root'] == $affiliate_id) {
            ...
            while (false !== ($affiliate_child = tep_db_fetch_array($affiliate_child_query))) {
                tep_db_query(
                    'UPDATE  '
                    . TABLE_AFFILIATE
                    . ' SET affiliate_root = '
                    . $affiliate_child['affiliate_id']
                    . ' WHERE affiliate_root =  '
                    . $affiliate['affiliate_root']
                    . '  AND affiliate_lft >= '
                    . $affiliate_child['affiliate_lft']
                    . ' AND affiliate_rgt <= '
                    . $affiliate_child['affiliate_rgt']
                    . ' '
                );
...
if (!function_exists('tep_draw_form')) {
    ...
    {
        $form = '<form name="' . tep_parse_input_field_data($name, ['"' => '&quot;']) . '" action="' . tep_parse_input_field_data($action, ['"' => '&quot;']) . '" method="' . tep_parse_input_field_data($method, ['"' => '&quot;']) . '"';
