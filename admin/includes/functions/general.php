<?php
/*
  $Id: general.php,v 1.1 2006/03/27 09:06:04 mikhail Exp $
...
function tep_get_path($current_category_id = '')
{
    ...
    if ($current_category_id == '') {
        ...
        if (count($cPath_array) == 0) {
            ...
            $cPath_new           = '';
            $last_category_query = tep_db_query("select parent_id from " . TABLE_CATEGORIES . " where categories_id = '" . (int)$cPath_array[(count($cPath_array) - 1)] . "'");
...
function tep_products_in_category_count($categories_id, $include_deactivated = false)
{
    ...
    if (tep_db_num_rows($childs_query)) {
        while (false !== ($childs = tep_db_fetch_array($childs_query))) {
...
function tep_get_address_formats()
{
    ...
    while (false !== ($address_format_values = tep_db_fetch_array($address_format_query))) {
        $address_format_array[] = [
...
function tep_get_system_information()
{
    ...
    [$system, $host, $kernel] = preg_preg_split('/[\s,]+/', @exec('uname -a'), 5);
    ...
    return [
        ...
        'db_version'  => 'MySQL ' . (function_exists('$GLOBALS['xoopsDB']->getServerVersion') ? $GLOBALS['xoopsDB']->getServerVersion() : ''),
        ...
        while (false !== ($categories = tep_db_fetch_array($categories_query))) {
            if ($categories['categories_id'] == '0') {
                ...
                $category_query             = tep_db_query(
                    "select cd.categories_name, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = '" . (int)$categories['categories_id'] . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "'"
                );
                $category                   = tep_db_fetch_array($category_query);
...
function tep_remove_product($product_id)
{
    ...
    while (false !== ($product_reviews = tep_db_fetch_array($product_reviews_query))) {
...
function tep_remove_order($order_id, $restock = false)
{
    if ($restock == 'on') {
        ...
        while (false !== ($order = tep_db_fetch_array($order_query))) {
...
function tep_cfg_pull_down_zone_classes($zone_class_id, $key = '')
{
    ...
    while (false !== ($zone_class = tep_db_fetch_array($zone_class_query))) {
        $zone_class_array[] = [
...
function tep_cfg_pull_down_order_statuses($order_status_id, $key = '')
{
    ...
    while (false !== ($statuses = tep_db_fetch_array($statuses_query))) {
        $statuses_array[] = [
