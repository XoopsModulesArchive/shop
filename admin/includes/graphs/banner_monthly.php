...
/*
  $Id: banner_monthly.php,v 1.1 2006/03/27 09:05:39 mikhail Exp $
...
$stats = [];
for ($i = 1; $i < 13; $i++) {
    $stats[] = [strftime('%b', mktime(0, 0, 0, $i)), '0', '0'];
}
...
while (false !== ($banner_stats = tep_db_fetch_array($banner_stats_query))) {
    $stats[($banner_stats['banner_month'] - 1)] = [strftime('%b', mktime(0, 0, 0, $banner_stats['banner_month'])), (($banner_stats['value']) ?: '0'), (($banner_stats['dvalue']) ?: '0')];
}
