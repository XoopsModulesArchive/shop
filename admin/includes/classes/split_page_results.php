...
/*
  $Id: split_page_results.php,v 1.1 2006/03/27 09:01:42 mikhail Exp $
...
class splitPageResults
{
    public function __construct(&$current_page_number, $max_rows_per_page, &$sql_query, &$query_num_rows)
    {
        ...
        $pos_to = mb_strlen($sql_query);

        $pos_from = mb_strpos($sql_query, ' from', 0);

        $pos_group_by = mb_strpos($sql_query, ' group by', $pos_from);

        if (($pos_group_by < $pos_to) && (false !== $pos_group_by)) {
            $pos_to = $pos_group_by;
        }

        $pos_having = mb_strpos($sql_query, ' having', $pos_from);

        if (($pos_having < $pos_to) && (false !== $pos_having)) {
            $pos_to = $pos_having;
        }

        $pos_order_by = mb_strpos($sql_query, ' order by', $pos_from);

        if (($pos_order_by < $pos_to) && (false !== $pos_order_by)) {
            $pos_to = $pos_order_by;
        }

        $reviews_count_query = tep_db_query('select count(*) as total ' . mb_substr($sql_query, $pos_from, ($pos_to - $pos_from)));
    ...
    public function display_links($query_numrows, $max_rows_per_page, $max_page_links, $current_page_number, $parameters = '', $page_name = 'page')
    {
        ...
        if (tep_not_null($parameters) && ('&' != mb_substr($parameters, -1))) {
            $parameters .= '&';
        }
        ...
        if ($num_pages > 1) {
            ...
            if (($current_page_number < $num_pages) && (1 != $num_pages)) {
            ...
            if ('' != $parameters) {
                if ('&' == mb_substr($parameters, -1)) {
                    $parameters = mb_substr($parameters, 0, -1);
                }
