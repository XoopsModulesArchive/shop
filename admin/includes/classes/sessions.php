<?php
/*
  $Id: sessions.php,v 1.1 2006/03/27 09:01:42 mikhail Exp $
...
class php3session
{
    ...
    {
        $this->mod_name = $this->saveHandler;
    }
}
...
class php3session_files
{
    ...
    public function read($sess_id)
    {
        ...
        $fp  = fopen($file, 'r') || die('Could not open session file (' . $file . ').');
    ...
    public function write($sess_id, $val)
    {
        ...
        $file = $session->save_path . '/sess_' . $sess_id;
        $fp   = fopen($file, 'w') || die('Could not write session file (' . $file . ')');
        $val  = fwrite($fp, $val);
        fclose($fp);
...
function session_set_saveHandler($open, $close, $read, $write, $destroy, $gc)
{
...
function session_encode()
{
    ...
    $serializer = '_' . $session->serializeHandler . '_encode';
    $ret        = $serializer();
...
function session_decode($data)
{
    ...
    $serializer = '_' . $session->serializeHandler . '_decode';
    $ret        = $serializer($data);
...
$session = new php3session();
$mod     = $session->saveHandler;
$$mod    = new $mod();

if ($session->auto_start) {
    $ret = session_start() || die('Session could not be started.');
}
