...
/*
  $Id: phplot.php,v 1.1 2006/03/27 09:01:42 mikhail Exp $
...
class PHPlot
{
    ...
    public $line_style = ['solid', 'solid', 'solid', 'dashed', 'dashed', 'solid']; //Solid or dashed lines
    ...
    public function __construct($which_width = 600, $which_height = 400, $which_output_file = '', $which_input_file = '')
    {
        ...
        if ((1 == $this->session_set) && ('' == $this->img)) {  //For sessions
            //Do nothing
        } else {
...
*************************************** */
    ...
    public function SetFileFormat($which_file_format)
    {
        ...
        $asked = mb_strtolower(trim($which_file_format));

        if (('jpg' == $asked) || ('png' == $asked) || ('gif' == $asked) || ('wbmp' == $asked)) {
    ...
    {
        $this->i_light = [194, 194, 194];

        $this->i_dark = [100, 100, 100];

        $this->SetPlotBgColor([222, 222, 222]);

        $this->SetBackgroundColor([200, 222, 222]); //can use rgb values or "name" values
        ...
        $this->SetLightGridColor([175, 175, 175]);

        $this->SetTickColor('black');

        $this->SetTitleColor([0, 0, 0]); // Can be array or name

        $this->data_color = ['blue', 'green', 'yellow', 'red', 'orange'];

        $this->error_bar_color = ['blue', 'green', 'yellow', 'red', 'orange'];

        $this->data_border_color = ['black'];
    ...
    {
        if ((0 == $this->browser_cache) && (0 == $this->is_inline)) { //Submitted by Thiemo Nagel
        ...
        switch ($this->file_format) {
            case 'png':
                ...
                if (1 == $this->is_inline && '' != $this->output_file) {
    ...
    public function DrawBackground()
    {
        ...
        if (0 == $this->background_done) { //Don't draw it twice if drawing two plots on one image
    ...
    public function DrawYLabel()
    {
        if (1 == $this->use_ttf) {
            ...
            $ypos = (($this->small_font_width * mb_strlen($this->y_label_txt) / 2.0) + $this->ytr(($this->plot_max_y + $this->plot_min_y) / 2.0));
    ...
    public function DrawText($which_font, $which_angle, $which_xpos, $which_ypos, $which_color, $which_size, $which_text, $which_halign = 'left', $which_valign = '')
    {
        if (1 == $this->use_ttf) {
            ...
            if (90 == $which_angle) {  //Vertical Code Submitted by Marlin Viss
                for ($i = 0, $iMax = count($str); $i < $iMax; $i++) {
                    imagestringup($this->img, $which_font, ($i * $height + $which_xpos), $which_ypos, $str[$i], $which_color);
                }
            } else {
                for ($i = 0, $iMax = count($str); $i < $iMax; $i++) {
                    if ('center' == $which_halign) {
                        $xpos = $which_xpos - mb_strlen($str[$i]) * $width / 2;
    ...
    public function FindDataLimits()
    {
        ...
        switch ($this->data_type) {
            case 'text-data':
                ...
                $miny = (float)$this->data_values[0][1];
                $maxy = $miny;
    ...
    public function SetMargins()
    {
        ...
        if (1 == $this->use_ttf) {
            ...
            $title_size = [imagefontwidth($this->title_font) * mb_strlen($this->title_txt), imagefontheight($this->title_font)];
    ...
    public function SetPlotAreaWorld($xmin, $ymin, $xmax, $ymax)
    {
        if (('' == $xmin) && ('' == $xmax)) {
            ...
            if ('text-data' == $this->data_type) { //labels for text-data is done at data drawing time for speed.
    ...
    public function SetYLabelWidth()
    {
        ...
        if (1 == $this->use_ttf) {
            ...
            $size[0] = mb_strlen($ylab) * $this->small_font_width * .6;
        }
    ...
    public function SetPointSize($which_ps)
    {
        ...
        $which_ps = (int) $which_ps;

        $this->point_size = $which_ps;

        if ('diamond' == $this->point_shape or 'triangle' == $this->point_shape) {
            if (0 != $this->point_size % 2) {
                $this->point_size++;
            }
        }
    ...
    public function SetDataType($which_dt)
    {
        ...
        if ('text-linear' == $which_dt) {
            $which_dt = 'text-data';
        }

        if ('linear-linear' == $which_dt) {
            $which_dt = 'data-data';
        }

        if ('linear-linear-error' == $which_dt) {
    ...
    public function SetRGBArray($which_color_array)
    {
        if (is_array($which_color_array)) {
            ...
            return true;
        } elseif (2 == $which_color_array) { //Use the small predefined color array
            $this->rgb_array = [
                'white' => [255, 255, 255],
'snow' => [255, 250, 250],
...
'aquamarine1' => [127, 255, 212],
    ...
    { //Color is passed in as anything
        [$r, $g, $b] = $this->SetRgbColor($which_color);  //Translate to RGB
    ...
    public function SetRgbColor($color_asked)
    {
        ...
        if (3 == count($color_asked)) { //already array of 3 rgb
            $ret_val = $color_asked;
        } else { // is asking for a color by string
            if ('#' == mb_substr($color_asked, 0, 1)) {  //asking in #FFFFFF format.
                $ret_val = [hexdec(mb_substr($color_asked, 1, 2)), hexdec(mb_substr($color_asked, 3, 2)), hexdec(mb_substr($color, 5, 2))];
            } else {
    ...
    public function SetDataColors($which_data, $which_border)
    {
        ...
        if (!$which_data) {
            $which_data = [[0, 255, 0], [0, 0, 248], 'yellow', [255, 0, 0], 'orange'];
    ...
    public function DrawXAxis()
    {
        ...
        //X Ticks and Labels
        if ('text-data' != $this->data_type) { //labels for text-data done at data drawing time for speed.
    ...
    public function DrawHorizontalTicks()
    {
        ...
        switch ($this->x_grid_label_type) {
            case 'title':
                ...
                break;
            case 'data':
                $xlab = number_format($this->plot_min_x, $this->x_precision, '.', ',') . (string)$this->si_units;
                break;
            case 'none':
                $xlab = '';
                break;
        ...
        if (90 == $this->x_datalabel_angle) {
            ...
            $ypos = ($this->small_font_width * mb_strlen($xlab) + $this->plot_area[3] + $this->small_font_height);

            imagestringup($this->img, $this->small_font, $xpos, $ypos, $xlab, $this->ndx_text_color);
        } else {
            $xpos = $this->plot_area[0] - $this->small_font_width * mb_strlen($xlab) / 2;
        ...
        while ($x_tmp <= $this->plot_max_x) {
            ...
            switch ($this->x_grid_label_type) {
                case 'title':
                    ...
                    break;
                case 'data':
                    $xlab = number_format($x_tmp, $this->x_precision, '.', ',') . (string)$this->si_units;
                    break;
                case 'none':
                    $xlab = '';
                    break;
            ...
            if (90 == $this->x_datalabel_angle) {  //Vertical Code Submitted by Marlin Viss
                imagestringup(
                    $this->img,
                    $this->small_font,
                    ($x_pixels - $this->small_font_height / 2),
                    ($this->small_font_width * mb_strlen($xlab) + $this->plot_area[3] + $this->small_font_height),
                    $xlab,
                    $this->ndx_text_color
                );
            } else {
                imagestring(
                    $this->img,
                    $this->small_font,
                    ($x_pixels - $this->small_font_width * mb_strlen($xlab) / 2),
    ...
    public function FormatYTickLabel($which_ylab)
    {
        switch ($this->y_grid_label_type) {
            case 'data':
                $ylab = number_format($which_ylab, $this->y_precision, '.', ',') . (string)$this->si_units;
                break;
            case 'none':
                $ylab = '';
                break;
            ...
            case 'right':
                ...
                $sstr = '%' . mb_strlen($this->plot_max_y) . 's';
    ...
    {  //ylab in world coord.
        ...
        if (('plotleft' == $this->vert_tick_position) || ('both' == $this->vert_tick_position)) {
            imageline(
                $this->img,
        ...
        if (('plotright' == $this->vert_tick_position) || ('both' == $this->vert_tick_position)) {
            imageline(
                $this->img,
        ...
        if (('yaxis' == $this->vert_tick_position)) {
            imageline(
                $this->img,
    ...
    {
        if (1 != $this->skip_top_tick) { //If tick increment doesn't hit the top
            //Left Top
        ...
        while ($y_tmp <= $this->plot_max_y) {
            ...
            if (('log' == $this->yscale_type) && (1 == $this->plot_min_y)
                && (0 == $delta_y % 10)
                && ($y_tmp == $this->plot_min_y)) {
    ...
    public function DrawDataLabel($lab, $x_world, $y_world)
    {
        ...
        if ($this->use_ttf) {
            ...
            $lab_size = [$this->small_font_width * mb_strlen($lab), $this->small_font_height * 3];

            if (90 == $this->x_datalabel_angle) {
                $y = $this->ytr($y_world) - $this->small_font_width * mb_strlen($lab); //in pixels
                ...
                $x = $this->xtr($x_world) - ($this->small_font_width * mb_strlen($lab)) / 2;
    ...
    public function DrawXDataLabel($xlab, $xpos)
    {
        ...
        if ($this->use_ttf) {
            ...
            $xlab_size = [imagefontwidth($this->axis_font) * mb_strlen($xlab), $this->small_font_height * 3];

            if (90 == $this->x_datalabel_angle) {
                $y = $this->plot_area[3] + imagefontwidth($this->axis_font) * mb_strlen($xlab); //in pixels
                ...
                $x = $xpos - (imagefontwidth($this->axis_font) * mb_strlen($xlab)) / 2;
    ...
    public function DrawDots()
    {
        ...
        while (list($j, $row) = each($this->data_values)) {
            ...
            while (list($k, $v) = each($row)) {
                if (0 == $k) {
                } elseif ((1 == $k) && ('data-data' == $this->data_type)) {
                    $xpos = $v;
                } else {
                    if ('text-data' == $this->data_type) {
                        $xpos = ($j + .5);
                    }
                    ...
                    if (('' != (string)$v)) {   //Allow for missing Y data
        ...
        while (list(, $row) = each($this->data_values)) {
            ...
            while (list($k, $v) = each($row)) {
                if (0 == $k) {
                    ...
                    if ((1 == $this->draw_x_data_labels)) { //See "labels_note1 above.
        ...
        while (list($j, $row) = each($this->data_values)) {
            ...
            while (list($k, $v) = each($row)) {
                if (0 == $k) {
                    ...
                    $xlab = mb_substr($v, 0, $this->x_datalabel_maxlength);
                } elseif (1 == $k) {
                    $x = $this->xtr($v);
    ...
    public function DrawLines()
    {
        ...
        while (list($j, $row) = each($this->data_values)) {
            ...
            while (list($k, $v) = each($row)) {
                ...
                if ((1 == $this->draw_x_data_labels) && (1 == $k)) { //See "labels_note1 above.
            ...
            while (list($k, $v) = each($row)) {
                if (0 == $k) {
                    ...
                    if (('' != (string)$v)) {   //Allow for missing Y data
                        ...
                        if ('1' == $this->draw_data_labels) {  //ajo
    ...
    public function DrawLegend($which_x1, $which_y1, $which_boxtype)
    {
        ...
        while (list(, $leg) = each($this->legend)) {
            ...
            imagestring(
                $this->img,
                $this->small_font,
                $box_start_x + $this->small_font_width * ($max_legend_length - mb_strlen($leg) + 1),
                $y_pos,
                $leg,
                $this->ndx_text_color
            );
    ...
    public function DrawGraph()
    {
        ...
        if (!is_array($this->data_values)) {
            ...
            if (!$this->data_color) {
                $this->SetDataColors(['blue', 'green', 'yellow', 'red', 'orange', 'blue'], ['black']);
            }
            ...
            if (1 == $this->draw_plot_area_background) {
            ...
            switch ($this->plot_type) {
                ...
                case 'lines':
                    ...
                    if ('text-data' == $this->data_type) {
                        $this->DrawLines();
                    } elseif ('data-data-error' == $this->data_type) {
                        $this->DrawLinesError();
                    } else {
                        $this->DrawLines();
                    }
