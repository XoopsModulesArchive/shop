...
/*
  $Id: affiliate_newsletters.php,v 2.00 2004/10/12
    ...
    switch ($action) {
        ...
        case 'update':
            if (isset($_POST['newsletter_id'])) {
                $affiliate_newsletter_id = tep_db_prepare_input($_POST['newsletter_id']);
            }
            ...
            if (false === $affiliate_newsletter_error) {
                $sql_data_array = [
                    'title' => $title,
'content' => $content,
...
'module' => '',
                    ...
                    if (isset($_GET['nID'])) {
                        ...
                        $nInfo->__construct($affiliate_newsletter);
                    } elseif ($_POST) {
                        $nInfo->__construct($_POST);
                    }

                    $file_extension = mb_substr($PHP_SELF, mb_strrpos($PHP_SELF, '.'));
                    ...
                    if ($dir = dir(DIR_WS_MODULES . 'newsletters/')) {
                        while ($file = $dir->read()) {
                            if (!is_dir(DIR_WS_MODULES . 'newsletters/' . $file)) {
                                if (mb_substr($file, mb_strrpos($file, '.')) == $file_extension) {
                    ...
                    for ($i = 0, $n = count($directory_array); $i < $n; $i++) {
                        $modules_array[] = ['id' => mb_substr($directory_array[$i], 0, mb_strrpos($directory_array[$i], '.')), 'text' => mb_substr($directory_array[$i], 0, mb_strrpos($directory_array[$i], '.'))];
                    } ?>
                    <tr>
                        <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                    </tr>
                    <tr><?php echo tep_draw_form('newsletter', FILENAME_AFFILIATE_NEWSLETTERS, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'action=' . $form_action);
                    ...
                    <tr>
                        <td>
                            <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                <tr>
                                    <td class="main" align="right"><?php echo(('insert' == $form_action) ? tep_image_submit('button_save.gif', IMAGE_SAVE) : tep_image_submit('button_update.gif', IMAGE_UPDATE)) . '&nbsp;&nbsp;<a href="' . tep_href_link(
                        FILENAME_AFFILIATE_NEWSLETTERS,
                        (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . (isset($_GET['nID']) ? 'nID=' . $_GET['nID'] : '')
                    ...
                    include DIR_WS_LANGUAGES . $language . '/modules/newsletters/' . $nInfo->module . mb_substr($PHP_SELF, mb_strrpos($PHP_SELF, '.'));

                    include DIR_WS_MODULES . 'newsletters/' . $nInfo->module . mb_substr($PHP_SELF, mb_strrpos($PHP_SELF, '.'));
                    ...
                    while (false !== ($affiliate_newsletters = tep_db_fetch_array($affiliate_newsletters_query))) {
                        if ((!isset($_GET['nID']) || (isset($_GET['nID']) && ($_GET['nID'] == $affiliate_newsletters['affiliate_newsletters_id']))) && !isset($nInfo) && ('new' != mb_substr($action, 0, 3))) {
                        ...
                        if (isset($nInfo) && is_object($nInfo) && ($affiliate_newsletters['affiliate_newsletters_id'] == $nInfo->affiliate_newsletters_id)) {
                            echo '                  <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(
                                FILENAME_AFFILIATE_NEWSLETTERS,
                                'page=' . $_GET['page'] . '&nID=' . $nInfo->affiliate_newsletters_id . '&action=preview'
                            ) . '\'">' . "\n";
                        } else {
                            echo '                  <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(
                                FILENAME_AFFILIATE_NEWSLETTERS,
                                'page=' . $_GET['page'] . '&nID=' . $affiliate_newsletters['affiliate_newsletters_id']
                            ) . '\'">' . "\n";
                        } ?>
                                                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $affiliate_newsletters['affiliate_newsletters_id'] . '&action=preview') . '">' . tep_image(
                            DIR_WS_ICONS . 'preview.gif',
                            ICON_PREVIEW
                        ) . '</a>&nbsp;' . $affiliate_newsletters['title']; ?></td>
                                                ...
                                                <td class="dataTableContent" align="center"><?php if ('1' == $affiliate_newsletters['status']) {
                            echo tep_image(DIR_WS_ICONS . 'tick.gif', ICON_TICK);
                        } else {
                            echo tep_image(DIR_WS_ICONS . 'cross.gif', ICON_CROSS);
                        } ?></td>
                                                <td class="dataTableContent" align="center"><?php if ($affiliate_newsletters['locked'] > 0) {
                            ...
                            echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $affiliate_newsletters['affiliate_newsletters_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
                        } ?>&nbsp;
                                                </td>
                                                </tr>
                                                <?php
                    ...
                    switch ($action) {
                                        case 'delete':
                                            ...
                                            $contents = ['form' => tep_draw_form('newsletters', FILENAME_AFFILIATE_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->affiliate_newsletters_id . '&action=deleteconfirm')];
                                            $contents[] = ['text' => TEXT_INFO_DELETE_INTRO];
                                            $contents[] = ['text' => '<br><b>' . $nInfo->title . '</b>'];
                                            $contents[] = [
                                                'align' => 'center',
'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_AFFILIATE_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' . tep_image_button(
    'button_cancel.gif',
    IMAGE_CANCEL
) . '</a>',
                                            ];
                                            ...
                                            if (is_object($nInfo)) {
                                                ...
                                                if ($nInfo->locked > 0) {
                                                    $contents[] = [
                                                        'align' => 'center',
'text' => '<a href="'
                                                                   . tep_href_link(FILENAME_AFFILIATE_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->affiliate_newsletters_id . '&action=new')
                                                                   . '">'
                                                                   . tep_image_button('button_edit.gif', IMAGE_EDIT)
                                                                   . '</a> <a href="'
                                                                   . tep_href_link(FILENAME_AFFILIATE_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->affiliate_newsletters_id . '&action=delete')
                                                                   . '">'
                                                                   . tep_image_button('button_delete.gif', IMAGE_DELETE)
                                                                   . '</a> <a href="'
                                                                   . tep_href_link(FILENAME_AFFILIATE_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->affiliate_newsletters_id . '&action=preview')
                                                                   . '">'
                                                                   . tep_image_button('button_preview.gif', IMAGE_PREVIEW)
                                                                   . '</a> <a href="'
                                                                   . tep_href_link(FILENAME_AFFILIATE_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->affiliate_newsletters_id . '&action=send')
                                                                   . '">'
                                                                   . tep_image_button('button_send.gif', IMAGE_SEND)
                                                                   . '</a> <a href="'
                                                                   . tep_href_link(FILENAME_AFFILIATE_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->affiliate_newsletters_id . '&action=unlock')
                                                                   . '">'
                                                    ...
                                                    $contents[] = [
                                                        'align' => 'center',
'text' => '<a href="' . tep_href_link(FILENAME_AFFILIATE_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->affiliate_newsletters_id . '&action=preview') . '">' . tep_image_button(
    'button_preview.gif',
    IMAGE_PREVIEW
) . '</a> <a href="' . tep_href_link(FILENAME_AFFILIATE_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->affiliate_newsletters_id . '&action=lock') . '">' . tep_image_button('button_lock.gif', IMAGE_LOCK) . '</a>',
                                                    ];
                                                }
