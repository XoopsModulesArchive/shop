...
/*
  $Id: tax_classes.php,v 1.1 2006/03/27 09:05:33 mikhail Exp $
    ...
    switch ($action) {
        ...
        case 'save':
            ...
            tep_db_query(
                ...
                . (int)$tax_class_id
                . "', tax_class_title = '"
                . tep_db_input($tax_class_title)
                . "', tax_class_description = '"
                . tep_db_input($tax_class_description)
                . "', last_modified = now() where tax_class_id = '"
                . (int)$tax_class_id
                . "'"
            );
                            ...
                            <tr>
                                <td valign="top">
                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                        ...
                                        while (false !== ($classes = tep_db_fetch_array($classes_query))) {
                                            if ((!isset($_GET['tID']) || (isset($_GET['tID']) && ($_GET['tID'] == $classes['tax_class_id']))) && !isset($tcInfo) && ('new' != mb_substr($action, 0, 3))) {
                                            ...
                                            <td class="dataTableContent" align="right"><?php if (isset($tcInfo) && is_object($tcInfo) && ($classes['tax_class_id'] == $tcInfo->tax_class_id)) {
                                                ...
                                                echo '<a href="' . tep_href_link(FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&tID=' . $classes['tax_class_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
                                            } ?>&nbsp;
                                            </td>
                                            </tr>
                                            <?php
                                ...
                                switch ($action) {
                                    case 'new':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_NEW_TAX_CLASS . '</b>'];

                                        $contents = ['form' => tep_draw_form('classes', FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&action=insert')];
                                        ...
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . tep_image_submit('button_insert.gif', IMAGE_INSERT) . '&nbsp;<a href="' . tep_href_link(FILENAME_TAX_CLASSES, 'page=' . $_GET['page']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>',
                                        ];
                                        break;
                                    case 'edit':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_EDIT_TAX_CLASS . '</b>'];

                                        $contents = ['form' => tep_draw_form('classes', FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&tID=' . $tcInfo->tax_class_id . '&action=save')];
                                        ...
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . '&nbsp;<a href="' . tep_href_link(FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&tID=' . $tcInfo->tax_class_id) . '">' . tep_image_button(
                                                'button_cancel.gif',
                                                IMAGE_CANCEL
                                            ) . '</a>',
                                        ];
                                        break;
                                    case 'delete':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_DELETE_TAX_CLASS . '</b>'];

                                        $contents = ['form' => tep_draw_form('classes', FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&tID=' . $tcInfo->tax_class_id . '&action=deleteconfirm')];
                                        $contents[] = ['text' => TEXT_INFO_DELETE_INTRO];
                                        $contents[] = ['text' => '<br><b>' . $tcInfo->tax_class_title . '</b>'];
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . '&nbsp;<a href="' . tep_href_link(FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&tID=' . $tcInfo->tax_class_id) . '">' . tep_image_button(
                                                'button_cancel.gif',
                                                IMAGE_CANCEL
                                            ) . '</a>',
                                        ];
                                    ...
                                    default:
                                        if (isset($tcInfo) && is_object($tcInfo)) {
                                            $heading[] = ['text' => '<b>' . $tcInfo->tax_class_title . '</b>'];

                                            $contents[] = [
                                                'align' => 'center',
'text' => '<a href="' . tep_href_link(FILENAME_TAX_CLASSES, 'page=' . $_GET['page'] . '&tID=' . $tcInfo->tax_class_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(
                                                    FILENAME_TAX_CLASSES,
