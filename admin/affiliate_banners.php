...
/*
  $Id: affiliate_banners.php,v 2.00 2004/10/12
...
if ($_GET['action']) {
    switch ($_GET['action']) {
        case 'setaffiliate_flag':
            if (('0' == $_GET['affiliate_flag']) || ('1' == $_GET['affiliate_flag'])) {
        ...
        case 'update':
            ...
            */
            if (($affiliate_banners_image) && ('none' != $affiliate_banners_image) && (is_uploaded_file($affiliate_banners_image))) {
                if (!is_writable(DIR_FS_CATALOG_IMAGES . $affiliate_banners_image_target)) {
            ...
            if (!$affiliate_banner_error) {
                ...
                $sql_data_array = [
                    'affiliate_banners_title' => $affiliate_banners_title,
...
'affiliate_status' => '1',
                    ...
                    if (1 == $affiliate_banners_id) {
                        tep_db_query('update ' . TABLE_AFFILIATE_BANNERS . ' set affiliate_banners_id = affiliate_banners_id + 1');
                    }

                    $messageStack->add_session(SUCCESS_BANNER_INSERTED, 'success');
                } elseif ('update' == $_GET['action']) {
                    $insert_sql_data = ['affiliate_date_status_change' => 'now()'];
        ...
        case 'deleteconfirm':
            ...
            if ('on' == $delete_image) {
                ...
                if (is_file(DIR_FS_CATALOG_IMAGES . $affiliate_banner['affiliate_banners_image'])) {
                    if (is_writable(DIR_FS_CATALOG_IMAGES . $affiliate_banner['affiliate_banners_image'])) {
        ...
        <td width="100%" valign="top">
            <table border="0" width="100%" cellspacing="0" cellpadding="2">
                ...
                if ('new' == $_GET['action']) {
                    ...
                    while (false !== ($groups = tep_db_fetch_array($groups_query))) {
                        $groups_array[] = ['id' => $groups['affiliate_banners_group'], 'text' => $groups['affiliate_banners_group']];
                    } ?>
                    <tr>
                        <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                    </tr>
                    <tr><?php echo tep_draw_form('new_banner', FILENAME_AFFILIATE_BANNER_MANAGER, 'page=' . $_GET['page'] . '&action=' . $form_action, 'post', 'enctype="multipart/form-data"');

                    if ('update' == $form_action) {
                        echo tep_draw_hidden_field('affiliate_banners_id', $abID);
                    } ?>
                        <td>
                            <table border="0" cellspacing="2" cellpadding="2">
                                ...
                                <tr>
                                    <td class="main" colspan=2>&nbsp;&nbsp;</b><?php echo '<a href="javascript:popupWindow(\'' . tep_href_link(FILENAME_AFFILIATE_VALIDPRODUCTS) . '\')"><b>' . TEXT_AFFILIATE_VALIDPRODUCTS . '</b></a>'; ?>
                                        &nbsp;&nbsp;<?php echo TEXT_AFFILIATE_INDIVIDUAL_BANNER_VIEW; ?></td>
                                </tr>
                                <tr>
                    ...
                    <tr>
                        <td>
                            <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                <tr>
                                    <td class="main" align="right" valign="top" nowrap><?php echo(('insert' == $form_action) ? tep_image_submit('button_insert.gif', IMAGE_INSERT) : tep_image_submit('button_update.gif', IMAGE_UPDATE)) . '&nbsp;&nbsp;<a href="' . tep_href_link(
                                    FILENAME_AFFILIATE_BANNER_MANAGER,
                                    'selected_box=affiliate&page=' . $_GET['page'] . '&abID=' . $_GET['abID']
                    ...
                    while (false !== ($affiliate_banners = tep_db_fetch_array($affiliate_banners_query))) {
                        $info_query = tep_db_query(
                            'select sum(affiliate_banners_shown) as affiliate_banners_shown, sum(affiliate_banners_clicks) as affiliate_banners_clicks from '
                                                    . TABLE_AFFILIATE_BANNERS_HISTORY
                                                    . " where affiliate_banners_id = '"
                                                    . $affiliate_banners['affiliate_banners_id']
                                                    . "'"
                        );

                        $info = tep_db_fetch_array($info_query);

                        if (((!$_GET['abID']) || ($_GET['abID'] == $affiliate_banners['affiliate_banners_id'])) && (!$abInfo) && ('new' != mb_substr($_GET['action'], 0, 3))) {
                        ...
                        if ((is_object($abInfo)) && ($affiliate_banners['affiliate_banners_id'] == $abInfo->affiliate_banners_id)) {
                            ...
                            echo '              <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(
                                ...
                                'abID=' . $affiliate_banners['affiliate_banners_id']
                            ) . '\'">' . "\n";
                        } ?>
                                                <td class="dataTableContent"><?php echo '<a href="javascript:popupImageWindow(\''
                                                                                        . FILENAME_AFFILIATE_POPUP_IMAGE
                                                                                        . '?banner='
                                                                                        . $affiliate_banners['affiliate_banners_id']
                                                                                        . '\')">'
                                                                                        . tep_image(DIR_WS_IMAGES . 'icon_popup.gif', ICON_PREVIEW)
                                                                                        . '</a>&nbsp;'
                                                                                        . $affiliate_banners['affiliate_banners_title']; ?></td>
                                                <td class="dataTableContent" align="right"><?php if ($affiliate_banners['affiliate_products_id'] > 0) {
                                                ...
                                                <td class="dataTableContent" align="right"><?php if ((is_object($abInfo)) && ($affiliate_banners['affiliate_banners_id'] == $abInfo->affiliate_banners_id)) {
                                                                                            ...
                                                                                            echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_BANNER_MANAGER, 'selected_box=affiliate&page=' . $_GET['page'] . '&abID=' . $affiliate_banners['affiliate_banners_id']) . '">' . tep_image(
                    ...
                    switch ($_GET['action']) {
                                        case 'delete':
                                            $heading[] = ['text' => '<b>' . $abInfo->affiliate_banners_title . '</b>'];

                                            $contents = ['form' => tep_draw_form('affiliate_banners', FILENAME_AFFILIATE_BANNER_MANAGER, 'page=' . $_GET['page'] . '&abID=' . $abInfo->affiliate_banners_id . '&action=deleteconfirm')];
                                            $contents[] = ['text' => TEXT_INFO_DELETE_INTRO];
                                            $contents[] = ['text' => '<br><b>' . $abInfo->affiliate_banners_title . '</b>'];
...
'text' => '<br>'
                                                           ...
                                                           . '&nbsp;<a href="'
                                                           . tep_href_link(FILENAME_AFFILIATE_BANNER_MANAGER, 'selected_box=affiliate&page=' . $_GET['page'] . '&abID=' . $_GET['abID'])
                                                           . '">'
                                        ...
                                        default:
                                            if (is_object($abInfo)) {
                                                ...
                                                $contents[] = [
                                                    'align' => 'center',
'text' => '<a href="' . tep_href_link(FILENAME_AFFILIATE_BANNER_MANAGER, 'selected_box=affiliate&page=' . $_GET['page'] . '&abID=' . $abInfo->affiliate_banners_id . '&action=new') . '">' . tep_image_button(
    'button_edit.gif',
    IMAGE_EDIT
) . '</a> <a href="' . tep_href_link(FILENAME_AFFILIATE_BANNER_MANAGER, 'selected_box=affiliate&page=' . $_GET['page'] . '&abID=' . $abInfo->affiliate_banners_id . '&action=delete') . '">' . tep_image_button(
                                                            'button_delete.gif',
                                                            IMAGE_DELETE
                                                        ) . '</a>',
                                                ];

                                                $contents[] = ['text' => $product_description['products_name']];

                                                $contents[] = ['text' => '<br>' . TEXT_BANNERS_DATE_ADDED . ' ' . tep_date_short($abInfo->affiliate_date_added)];

                                                $contents[] = ['text' => '' . sprintf(TEXT_BANNERS_STATUS_CHANGE, tep_date_short($abInfo->affiliate_date_status_change))];
                                            }
                                            break;
                                    }
