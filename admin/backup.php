...
/*
  $Id: backup.php,v 1.1 2006/03/27 09:05:33 mikhail Exp $
    ...
    switch ($action) {
        ...
        case 'backupnow':
            ...
            $schema = '# osCommerce, Open Source E-Commerce Solutions'
                      ...
                      . "\n"
                      . '# Backup Date: '
                      . date(PHP_DATE_TIME_FORMAT)
                      . "\n\n";
            fwrite($fp, $schema);

            $tables_query = tep_db_query('show tables');
            while (false !== ($tables = tep_db_fetch_array($tables_query))) {
                [, $table] = each($tables);
                ...
                while (false !== ($fields = tep_db_fetch_array($fields_query))) {
                    $table_list[] = $fields['Field'];

                    $schema .= '  ' . $fields['Field'] . ' ' . $fields['Type'];

                    if (mb_strlen($fields['Default']) > 0) {
                ...
                while (false !== ($keys = tep_db_fetch_array($keys_query))) {
                    $kname = $keys['Key_name'];
...
'columns' => [],
                ...
                while (false !== ($rows = tep_db_fetch_array($rows_query))) {
        ...
        case 'restorelocalnow':
            ...
            if ('restorenow' == $action) {
                $read_from = $_GET['file'];

                if (file_exists(DIR_FS_BACKUP . $_GET['file'])) {
                    $restore_file = DIR_FS_BACKUP . $_GET['file'];

                    $extension = mb_substr($_GET['file'], -3);

                    if (('sql' == $extension) || ('.gz' == $extension) || ('zip' == $extension)) {
                        switch ($extension) {
                            case 'sql':
                                ...
                                break;
                            case '.gz':
                                $restore_from = mb_substr($restore_file, 0, -3);
                                exec(LOCAL_EXE_GUNZIP . ' ' . $restore_file . ' -c > ' . $restore_from);
                                $remove_raw = true;
                                break;
                            case 'zip':
                                $restore_from = mb_substr($restore_file, 0, -4);
            ...
            } elseif ('restorelocalnow' == $action) {
                $sql_file = new upload('sql_file');

                if (true === $sql_file->parse()) {
            ...
            if (isset($restore_query)) {
                ...
                $sql_length = mb_strlen($restore_query);

                $pos = mb_strpos($restore_query, ';');

                for ($i = $pos; $i < $sql_length; $i++) {
                    if ('#' == $restore_query[0]) {
                        $restore_query = ltrim(mb_substr($restore_query, mb_strpos($restore_query, "\n")));

                        $sql_length = mb_strlen($restore_query);

                        $i = mb_strpos($restore_query, ';') - 1;
                    ...
                    if ("\n" == $restore_query[($i + 1)]) {
                        for ($j = ($i + 2); $j < $sql_length; $j++) {
                            if ('' != trim($restore_query[$j])) {
                                $next = mb_substr($restore_query, $j, 6);
                                ...
                                if ('#' == $next[0]) {
                                    ...
                                    $query = mb_substr($restore_query, 0, $i + 1);

                                    $restore_query = mb_substr($restore_query, $k);
                        ...
                        if ((eregi('create', $next)) || (eregi('insert', $next)) || (eregi('drop t', $next))) {
                            ...
                            $sql_array[] = mb_substr($restore_query, 0, $i);

                            $restore_query = ltrim(mb_substr($restore_query, $i + 1));
                ...
                if (isset($remove_raw) && (true === $remove_raw)) {
                    unlink($restore_from);
                }
        ...
        case 'download':
            ...
            if (('zip' == $extension) || ('.gz' == $extension) || ('sql' == $extension)) {
                    ...
                    <td>
                        <table border="0" width="100%" cellspacing="0" cellpadding="0">
                            <tr>
                                    ...
                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                        ...
                                        if (true === $dir_ok) {
                                            ...
                                            for ($i = 0, $n = count($contents); $i < $n; $i++) {
                                                ...
                                                if ((!isset($_GET['file']) || (isset($_GET['file']) && ($_GET['file'] == $entry))) && !isset($buInfo) && ('backup' != $action) && ('restorelocal' != $action)) {
                                                ...
                                                <td class="dataTableContent" onclick="document.location.href='<?php echo tep_href_link(FILENAME_BACKUP, $onclick_link); ?>'"><?php echo '<a href="' . tep_href_link(FILENAME_BACKUP, 'action=download&file=' . $entry) . '">' . tep_image(
                                                    DIR_WS_ICONS . 'file_download.gif',
                                                    ICON_FILE_DOWNLOAD
                                                ) . '</a>&nbsp;' . $entry; ?></td>
                                        ...
                                        <tr>
                                            ...
                                            <td align="right" class="smallText"><?php if (('backup' != $action) && (isset($dir))) {
                                            echo '<a href="' . tep_href_link(FILENAME_BACKUP, 'action=backup') . '">' . tep_image_button('button_backup.gif', IMAGE_BACKUP) . '</a>';
                                        }
                                                if (('restorelocal' != $action) && isset($dir)) {
                                                    echo '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_BACKUP, 'action=restorelocal') . '">' . tep_image_button('button_restore.gif', IMAGE_RESTORE) . '</a>';
                                                } ?></td>
                                        </tr>
                                        <?php
                                ...
                                switch ($action) {
                                    case 'backup':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_NEW_BACKUP . '</b>'];

                                        $contents = ['form' => tep_draw_form('backup', FILENAME_BACKUP, 'action=backupnow')];
                                        ...
                                        $contents[] = ['align' => 'center', 'text' => '<br>' . tep_image_submit('button_backup.gif', IMAGE_BACKUP) . '&nbsp;<a href="' . tep_href_link(FILENAME_BACKUP) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'];
                                        break;
                                    case 'restore':
                                        ...
                                        $contents[] = [
                                            'text' => tep_break_string(
                                                sprintf(TEXT_INFO_RESTORE, DIR_FS_BACKUP . ((TEXT_NO_EXTENSION != $buInfo->compression) ? mb_substr($buInfo->file, 0, mb_strrpos($buInfo->file, '.')) : $buInfo->file), (TEXT_NO_EXTENSION != $buInfo->compression) ? TEXT_INFO_UNPACK : ''),
                                                35,
                                                ' '
                                            ),
                                        ];
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br><a href="' . tep_href_link(FILENAME_BACKUP, 'file=' . $buInfo->file . '&action=restorenow') . '">' . tep_image_button('button_restore.gif', IMAGE_RESTORE) . '</a>&nbsp;<a href="' . tep_href_link(
    FILENAME_BACKUP,
    'file=' . $buInfo->file
                                        ...
                                        break;
                                    case 'restorelocal':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_RESTORE_LOCAL . '</b>'];

                                        $contents = ['form' => tep_draw_form('restore', FILENAME_BACKUP, 'action=restorelocalnow', 'post', 'enctype="multipart/form-data"')];
                                        ...
                                        $contents[] = ['align' => 'center', 'text' => '<br>' . tep_image_submit('button_restore.gif', IMAGE_RESTORE) . '&nbsp;<a href="' . tep_href_link(FILENAME_BACKUP) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'];
                                        break;
                                    case 'delete':
                                        $heading[] = ['text' => '<b>' . $buInfo->date . '</b>'];

                                        $contents = ['form' => tep_draw_form('delete', FILENAME_BACKUP, 'file=' . $buInfo->file . '&action=deleteconfirm')];
                                        $contents[] = ['text' => TEXT_DELETE_INTRO];
                                        $contents[] = ['text' => '<br><b>' . $buInfo->file . '</b>'];
                                        $contents[] = ['align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_BACKUP, 'file=' . $buInfo->file) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'];
                                        break;
                                    default:
                                        if (isset($buInfo) && is_object($buInfo)) {
                                            ...
                                            $contents[] = [
                                                'align' => 'center',
'text' => '<a href="' . tep_href_link(FILENAME_BACKUP, 'file=' . $buInfo->file . '&action=restore') . '">' . tep_image_button('button_restore.gif', IMAGE_RESTORE) . '</a> <a href="' . tep_href_link(
    FILENAME_BACKUP,
