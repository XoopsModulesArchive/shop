...
/*
  $Id: zones.php,v 1.1 2006/03/27 09:05:33 mikhail Exp $
                            ...
                            <tr>
                                <td valign="top">
                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                        ...
                                        while (false !== ($zones = tep_db_fetch_array($zones_query))) {
                                            if ((!isset($_GET['cID']) || (isset($_GET['cID']) && ($_GET['cID'] == $zones['zone_id']))) && !isset($cInfo) && ('new' != mb_substr($action, 0, 3))) {
                                            ...
                                            if (isset($cInfo) && is_object($cInfo) && ($zones['zone_id'] == $cInfo->zone_id)) {
                                                ...
                                                echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\''
                                                     . tep_href_link(FILENAME_ZONES, 'page=' . $_GET['page'] . '&cID=' . $zones['zone_id'])
                                                     . '\'">'
                                                     . "\n";
                                            } ?>
                                            ...
                                            <td class="dataTableContent" align="right"><?php if (isset($cInfo) && is_object($cInfo) && ($zones['zone_id'] == $cInfo->zone_id)) {
                                                ...
                                                echo '<a href="' . tep_href_link(FILENAME_ZONES, 'page=' . $_GET['page'] . '&cID=' . $zones['zone_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
                                            } ?>&nbsp;
                                            </td>
                                            </tr>
                                            <?php
                                ...
                                switch ($action) {
                                    case 'new':
                                        ...
                                        $contents = ['form' => tep_draw_form('zones', FILENAME_ZONES, 'page=' . $_GET['page'] . '&action=insert')];
                                        ...
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . tep_image_submit('button_insert.gif', IMAGE_INSERT) . '&nbsp;<a href="' . tep_href_link(FILENAME_ZONES, 'page=' . $_GET['page']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>',
                                        ];
                                        break;
                                    case 'edit':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_EDIT_ZONE . '</b>'];

                                        $contents = ['form' => tep_draw_form('zones', FILENAME_ZONES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->zone_id . '&action=save')];
                                        ...
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . '&nbsp;<a href="' . tep_href_link(FILENAME_ZONES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->zone_id) . '">' . tep_image_button(
                                                'button_cancel.gif',
                                                IMAGE_CANCEL
                                            ) . '</a>',
                                        ];
                                    ...
                                    case 'delete':
                                        ...
                                        $contents = ['form' => tep_draw_form('zones', FILENAME_ZONES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->zone_id . '&action=deleteconfirm')];
                                        $contents[] = ['text' => TEXT_INFO_DELETE_INTRO];
                                        $contents[] = ['text' => '<br><b>' . $cInfo->zone_name . '</b>'];
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . '&nbsp;<a href="' . tep_href_link(FILENAME_ZONES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->zone_id) . '">' . tep_image_button(
                                                'button_cancel.gif',
                                                IMAGE_CANCEL
                                            ) . '</a>',
                                        ];
                                    ...
                                    default:
                                        if (isset($cInfo) && is_object($cInfo)) {
                                            ...
                                            $contents[] = [
                                                'align' => 'center',
'text' => '<a href="' . tep_href_link(FILENAME_ZONES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->zone_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(
                                                    FILENAME_ZONES,
