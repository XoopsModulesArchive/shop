...
/*
  $Id: products_expected.php,v 1.1 2006/03/27 09:05:33 mikhail Exp $
                            ...
                            <tr>
                                <td valign="top">
                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                        ...
                                        <?php
                                        $products_query_raw = 'select pd.products_id, pd.products_name, p.products_date_available from '
                                                              . TABLE_PRODUCTS_DESCRIPTION
                                                              . ' pd, '
                                                              . TABLE_PRODUCTS
                                                              . " p where p.products_id = pd.products_id and p.products_date_available != '' and pd.language_id = '"
                                                              . (int)$languages_id
                                        ...
                                        while (false !== ($products = tep_db_fetch_array($products_query))) {
                                            ...
                                            <td class="dataTableContent" align="right"><?php if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo->products_id)) {
                                                ...
                                                echo '<a href="' . tep_href_link(FILENAME_PRODUCTS_EXPECTED, 'page=' . $_GET['page'] . '&pID=' . $products['products_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
                                            } ?>&nbsp;
                                            </td>
                                            </tr>
                                            <?php
                                ...
                                if (isset($pInfo) && is_object($pInfo)) {
                                    $heading[] = ['text' => '<b>' . $pInfo->products_name . '</b>'];

                                    $contents[] = ['align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'pID=' . $pInfo->products_id . '&action=new_product') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a>'];
