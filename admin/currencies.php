...
/*
  $Id: currencies.php,v 1.1 2006/03/27 09:05:33 mikhail Exp $
...
'value' => $value,
        ...
        case 'update':
            ...
            while (false !== ($currency = tep_db_fetch_array($currency_query))) {
                            ...
                            <tr>
                                <td valign="top">
                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                        ...
                                        while (false !== ($currency = tep_db_fetch_array($currency_query))) {
                                            if ((!isset($_GET['cID']) || (isset($_GET['cID']) && ($_GET['cID'] == $currency['currencies_id']))) && !isset($cInfo) && ('new' != mb_substr($action, 0, 3))) {
                                            ...
                                            <td class="dataTableContent" align="right"><?php if (isset($cInfo) && is_object($cInfo) && ($currency['currencies_id'] == $cInfo->currencies_id)) {
                                                ...
                                                echo '<a href="' . tep_href_link(FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . $currency['currencies_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
                                            } ?>&nbsp;
                                            </td>
                                            </tr>
                                            <?php
                                                ...
                                                <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                                    ...
                                                    if (empty($action)) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            echo '<a href="'
                                                                         . tep_href_link(FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id . '&action=update')
                                                                         . '">'
                                                                         . tep_image_button('button_update_currencies.gif', IMAGE_UPDATE_CURRENCIES)
                                                                         . '</a>';
                                                        } ?></td>
                                                            <td align="right"><?php echo '<a href="'
                                                                                         . tep_href_link(FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id . '&action=new')
                                                                                         . '">'
                                                                                         . tep_image_button('button_new_currency.gif', IMAGE_NEW_CURRENCY)
                                                                                         . '</a>'; ?></td>
                                                        </tr>
                                                        <?php
                                                    }
                                ...
                                switch ($action) {
                                    case 'new':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_NEW_CURRENCY . '</b>'];

                                        $contents = ['form' => tep_draw_form('currencies', FILENAME_CURRENCIES, 'page=' . $_GET['page'] . (isset($cInfo) ? '&cID=' . $cInfo->currencies_id : '') . '&action=insert')];
                                        ...
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . tep_image_submit('button_insert.gif', IMAGE_INSERT) . ' <a href="' . tep_href_link(FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . $_GET['cID']) . '">' . tep_image_button(
    'button_cancel.gif',
    IMAGE_CANCEL
) . '</a>',
                                        ];
                                        break;
                                    case 'edit':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_EDIT_CURRENCY . '</b>'];

                                        $contents = ['form' => tep_draw_form('currencies', FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id . '&action=save')];
                                        ...
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id) . '">' . tep_image_button(
    'button_cancel.gif',
    IMAGE_CANCEL
) . '</a>',
                                        ];
                                        break;
                                    case 'delete':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_DELETE_CURRENCY . '</b>'];

                                        $contents[] = ['text' => TEXT_INFO_DELETE_INTRO];
                                        $contents[] = ['text' => '<br><b>' . $cInfo->title . '</b>'];
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . (($remove_currency) ? '<a href="' . tep_href_link(FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id . '&action=deleteconfirm') . '">' . tep_image_button(
    'button_delete.gif',
    IMAGE_DELETE
) . '</a>' : '') . ' <a href="' . tep_href_link(FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>',
                                        ];
                                        break;
                                    default:
                                        if (is_object($cInfo)) {
                                            $heading[] = ['text' => '<b>' . $cInfo->title . '</b>'];

                                            $contents[] = [
                                                'align' => 'center',
'text' => '<a href="' . tep_href_link(FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(
    FILENAME_CURRENCIES,
