<?php
/*
  $Id: file_manager.php,v 1.1 2006/03/27 09:05:33 mikhail Exp $
...
if (!preg_match('^' . DIR_FS_DOCUMENT_ROOT, $current_path)) {
    $current_path = DIR_FS_DOCUMENT_ROOT;
}
    ...
    switch ($action) {
        ...
        case 'save':
            if ($fp = fopen($current_path . '/' . $_POST['filename'], 'w+')) {
                fwrite($fp, stripslashes($_POST['file_contents']));
                fclose($fp);
        ...
        case 'edit':
            ...
            $file_writeable = true;
            if (!is_writable($current_path . '/' . $_GET['info'])) {
...
$goto_array          = [['id' => DIR_FS_DOCUMENT_ROOT, 'text' => $in_directory]];
for ($i = 0, $n = count($current_path_array); $i < $n; $i++) {
    if ((isset($document_root_array[$i]) && ($current_path_array[$i] != $document_root_array[$i])) || !isset($document_root_array[$i])) {
        $goto_array[] = ['id' => implode('/', array_slice($current_path_array, 0, $i + 1)), 'text' => $current_path_array[$i]];
    }
}
?>
        ...
        <td width="100%" valign="top">
            <table border="0" width="100%" cellspacing="0" cellpadding="2">
                ...
                if ((($action == 'new_file') && ($directory_writeable === true)) || ($action == 'edit')) {
                    if (isset($_GET['info']) && false !== strpos($_GET['info'], '..')) {
                        tep_redirect(tep_href_link(FILENAME_FILE_MANAGER));
                    }
                    ...
                    if ($action == 'new_file') {
                        ...
                        if ($file_array = file($current_path . '/' . $_GET['info'])) {
                            $file_contents = htmlspecialchars(implode('', $file_array), ENT_QUOTES | ENT_HTML5);
                        }
                    ...
                    <tr><?php echo tep_draw_form('new_file', FILENAME_FILE_MANAGER, 'action=save'); ?>
                        <td>
                            <table border="0" cellspacing="0" cellpadding="2">
                                ...
                                <tr>
                                    <td align="right" class="main" colspan="2"><?php if ($file_writeable === true) {
                                            echo tep_image_submit('button_save.gif', IMAGE_SAVE) . '&nbsp;';
                                        }
                                        echo '<a href="' . tep_href_link(FILENAME_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?></td>
                                </tr>
                            </table>
                        </td>
                        </form></tr>
                        ...
                        <td>
                            <table border="0" width="100%" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td valign="top">
                                        <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                            ...
                                            for ($i = 0, $n = count($contents); $i < $n; $i++) {
                                                ...
                                                <td class="dataTableContent" align="right"><?php if ($contents[$i]['name'] != '..') {
                                                    ...
                                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                                        <tr valign="top">
                                                            ...
                                                            <td class="smallText" align="right"><?php echo '<a href="'
                                                                                                           . tep_href_link(FILENAME_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) . '&' : '') . 'action=upload')
                                                                                                           . '">'
                                                                                                           . tep_image_button(
                                                                                                               'button_upload.gif',
                                                                                                               IMAGE_UPLOAD
                                                                                                           ...
                                                                                                           . '</a>&nbsp;<a href="'
                                                                                                           . tep_href_link(FILENAME_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) . '&' : '') . 'action=new_file')
                                                                                                           . '">'
                                                                                                           . tep_image_button(
                                                                                                               'button_new_file.gif',
                                                                                                               IMAGE_NEW_FILE
                                                                                                           ...
                                                                                                           . '</a>&nbsp;<a href="'
                                                                                                           . tep_href_link(FILENAME_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) . '&' : '') . 'action=new_folder')
                                                                                                           . '">'
                                                                                                           . tep_image_button('button_new_folder.gif', IMAGE_NEW_FOLDER)
                                                                                                           . '</a>'; ?></td>
                                                        </tr>
                                                    </table>
                                                </td>
                                    ...
                                    switch ($action) {
                                        case 'delete':
                                            $heading[] = ['text' => '<b>' . $fInfo->name . '</b>'];

                                            $contents   = ['form' => tep_draw_form('file', FILENAME_FILE_MANAGER, 'info=' . urlencode($fInfo->name) . '&action=deleteconfirm')];
                                            $contents[] = ['text' => TEXT_DELETE_INTRO];
                                            $contents[] = ['text' => '<br><b>' . $fInfo->name . '</b>'];
                                            $contents[] = [
                                                'align' => 'center',
                                                'text'  => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_FILE_MANAGER, (tep_not_null($fInfo->name) ? 'info=' . urlencode($fInfo->name) : '')) . '">' . tep_image_button(
                                                        'button_cancel.gif',
                                                        IMAGE_CANCEL
                                                    ) . '</a>',
                                            ];
                                            break;
                                        case 'new_folder':
                                            $heading[] = ['text' => '<b>' . TEXT_NEW_FOLDER . '</b>'];

                                            $contents   = ['form' => tep_draw_form('folder', FILENAME_FILE_MANAGER, 'action=insert')];
                                            $contents[] = ['text' => TEXT_NEW_FOLDER_INTRO];
                                            $contents[] = ['text' => '<br>' . TEXT_FILE_NAME . '<br>' . tep_draw_input_field('folder_name')];
                                            $contents[] = [
                                                'align' => 'center',
                                                'text'  => '<br>' . (($directory_writeable === true) ? tep_image_submit('button_save.gif', IMAGE_SAVE) : '') . ' <a href="' . tep_href_link(
                                                        FILENAME_FILE_MANAGER,
                                                        (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')
                                                    ) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>',
                                            ];
                                            break;
                                        case 'upload':
                                            $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_UPLOAD . '</b>'];

                                            $contents   = ['form' => tep_draw_form('file', FILENAME_FILE_MANAGER, 'action=processuploads', 'post', 'enctype="multipart/form-data"')];
                                            $contents[] = ['text' => TEXT_UPLOAD_INTRO];
                                            ...
                                            $contents[] = ['text' => '<br>' . $file_upload];
                                            $contents[] = [
                                                'align' => 'center',
                                                'text'  => '<br>' . (($directory_writeable === true) ? tep_image_submit('button_upload.gif', IMAGE_UPLOAD) : '') . ' <a href="' . tep_href_link(
                                                        FILENAME_FILE_MANAGER,
                                                        (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')
                                                    ) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>',
                                            ];
                                        ...
                                        default:
                                            if (isset($fInfo) && is_object($fInfo)) {
                                                ...
                                                if (!$fInfo->is_dir) {
                                                    $contents[] = ['align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_FILE_MANAGER, 'info=' . urlencode($fInfo->name) . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a>'];
                                                }
                                                $contents[] = ['text' => '<br>' . TEXT_FILE_NAME . ' <b>' . $fInfo->name . '</b>'];
                                                if (!$fInfo->is_dir) {
                                                    $contents[] = ['text' => '<br>' . TEXT_FILE_SIZE . ' <b>' . $fInfo->size . '</b>'];
                                                }
                                                $contents[] = ['text' => '<br>' . TEXT_LAST_MODIFIED . ' ' . $fInfo->last_modified];
                                            }
                                    }
