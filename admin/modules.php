...
/*
  $Id: modules.php,v 1.1 2006/03/27 09:05:33 mikhail Exp $
                                ...
                                <td valign="top">
                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                        ...
                                        for ($i = 0, $n = count($directory_array); $i < $n; $i++) {
                                            ...
                                            $class = mb_substr($file, 0, mb_strrpos($file, '.'));
...
'status' => $module->check(),
                                                ...
                                                <td class="dataTableContent" align="right"><?php if (is_numeric($module->sort_order)) {
                                        ...
                                        if (tep_db_num_rows($check_query)) {
                                            ...
                                            tep_db_query(
                                                'insert into ' . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Installed Modules', '" . $module_key . "', '" . implode(
                                                    ';',
                                                    $installed_modules
                                ...
                                switch ($action) {
                                    case 'edit':
                                        ...
                                        $keys = mb_substr($keys, 0, mb_strrpos($keys, '<br><br>'));

                                        $heading[] = ['text' => '<b>' . $mInfo->title . '</b>'];

                                        $contents = ['form' => tep_draw_form('modules', FILENAME_MODULES, 'set=' . $set . '&module=' . $_GET['module'] . '&action=save')];
                                        $contents[] = ['text' => $keys];
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $_GET['module']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>',
                                        ];
                                        break;
                                    default:
                                        $heading[] = ['text' => '<b>' . $mInfo->title . '</b>'];
                                        ...
                                        if ('1' == $mInfo->status) {
                                            ...
                                            $contents[] = [
                                                'align' => 'center',
'text' => '<a href="' . tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=remove') . '">' . tep_image_button('button_module_remove.gif', IMAGE_MODULE_REMOVE) . '</a> <a href="' . tep_href_link(
                                                    FILENAME_MODULES,
                                                    'set=' . $set . (isset($_GET['module']) ? '&module=' . $_GET['module'] : '') . '&action=edit'
                                            ...
                                            $contents[] = ['align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=install') . '">' . tep_image_button('button_module_install.gif', IMAGE_MODULE_INSTALL) . '</a>'];
