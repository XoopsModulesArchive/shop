...
/*
  $Id: languages.php,v 1.1 2006/03/27 09:05:33 mikhail Exp $
    ...
    switch ($action) {
        case 'insert':
            ...
            while (false !== ($products = tep_db_fetch_array($products_query))) {
                tep_db_query(
                    'insert into ' . TABLE_PRODUCTS_DESCRIPTION . " (products_id, language_id, products_name, products_description, products_url) values ('" . (int)$products['products_id'] . "', '" . (int)$insert_id . "', '" . tep_db_input($products['products_name']) . "', '" . tep_db_input(
                        $products['products_description']
                    ) . "', '" . tep_db_input($products['products_url']) . "')"
                );
            }
            ...
            while (false !== ($products_options = tep_db_fetch_array($products_options_query))) {
            ...
            while (false !== ($products_options_values = tep_db_fetch_array($products_options_values_query))) {
                tep_db_query(
                    'insert into ' . TABLE_PRODUCTS_OPTIONS_VALUES . " (products_options_values_id, language_id, products_options_values_name) values ('" . (int)$products_options_values['products_options_values_id'] . "', '" . (int)$insert_id . "', '" . tep_db_input(
        ...
        case 'save':
            ...
            tep_db_query(
                ...
                . tep_db_input($image)
                . "', directory = '"
                . tep_db_input($directory)
                . "', sort_order = '"
                . tep_db_input($sort_order)
                . "' where languages_id = '"
                . (int)$lID
                . "'"
            );
                            ...
                            <tr>
                                <td valign="top">
                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                        ...
                                        while (false !== ($languages = tep_db_fetch_array($languages_query))) {
                                            if ((!isset($_GET['lID']) || (isset($_GET['lID']) && ($_GET['lID'] == $languages['languages_id']))) && !isset($lInfo) && ('new' != mb_substr($action, 0, 3))) {
                                            ...
                                            <td class="dataTableContent" align="right"><?php if (isset($lInfo) && is_object($lInfo) && ($languages['languages_id'] == $lInfo->languages_id)) {
                                                ...
                                                echo '<a href="' . tep_href_link(FILENAME_LANGUAGES, 'page=' . $_GET['page'] . '&lID=' . $languages['languages_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
                                            } ?>&nbsp;
                                            </td>
                                            </tr>
                                            <?php
                                            ...
                                            <td colspan="3">
                                                <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                                    ...
                                                    if (empty($action)) {
                                                        ?>
                                                        <tr>
                                                            <td align="right" colspan="2"><?php echo '<a href="' . tep_href_link(FILENAME_LANGUAGES, 'page=' . $_GET['page'] . '&lID=' . $lInfo->languages_id . '&action=new') . '">' . tep_image_button(
                                ...
                                switch ($action) {
                                    case 'new':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_NEW_LANGUAGE . '</b>'];

                                        $contents = ['form' => tep_draw_form('languages', FILENAME_LANGUAGES, 'action=insert')];
                                        ...
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . tep_image_submit('button_insert.gif', IMAGE_INSERT) . ' <a href="' . tep_href_link(FILENAME_LANGUAGES, 'page=' . $_GET['page'] . '&lID=' . $_GET['lID']) . '">' . tep_image_button(
                                                'button_cancel.gif',
                                                IMAGE_CANCEL
                                            ) . '</a>',
                                        ];
                                        break;
                                    case 'edit':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_EDIT_LANGUAGE . '</b>'];

                                        $contents = ['form' => tep_draw_form('languages', FILENAME_LANGUAGES, 'page=' . $_GET['page'] . '&lID=' . $lInfo->languages_id . '&action=save')];
                                        ...
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_LANGUAGES, 'page=' . $_GET['page'] . '&lID=' . $lInfo->languages_id) . '">' . tep_image_button(
                                                'button_cancel.gif',
                                                IMAGE_CANCEL
                                            ) . '</a>',
                                        ];
                                        break;
                                    case 'delete':
                                        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_DELETE_LANGUAGE . '</b>'];

                                        $contents[] = ['text' => TEXT_INFO_DELETE_INTRO];
                                        $contents[] = ['text' => '<br><b>' . $lInfo->name . '</b>'];
                                        $contents[] = [
                                            'align' => 'center',
'text' => '<br>' . (($remove_language) ? '<a href="'
                                                                                        . tep_href_link(FILENAME_LANGUAGES, 'page=' . $_GET['page'] . '&lID=' . $lInfo->languages_id . '&action=deleteconfirm')
                                                                                        . '">'
                                                                                        . tep_image_button('button_delete.gif', IMAGE_DELETE)
                                                                                        . '</a>' : '') . ' <a href="' . tep_href_link(FILENAME_LANGUAGES, 'page=' . $_GET['page'] . '&lID=' . $lInfo->languages_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>',
                                        ];
                                        break;
                                    default:
                                        if (is_object($lInfo)) {
                                            $heading[] = ['text' => '<b>' . $lInfo->name . '</b>'];

                                            $contents[] = [
                                                'align' => 'center',
'text' => '<a href="' . tep_href_link(FILENAME_LANGUAGES, 'page=' . $_GET['page'] . '&lID=' . $lInfo->languages_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(
                                                    FILENAME_LANGUAGES,
                                                    'page=' . $_GET['page'] . '&lID=' . $lInfo->languages_id . '&action=delete'
                                                ) . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a> <a href="' . tep_href_link(FILENAME_DEFINE_LANGUAGE, 'lngdir=' . $lInfo->directory) . '">' . tep_image_button('button_details.gif', IMAGE_DETAILS) . '</a>',
                                            ];
                                            ...
                                            $contents[] = ['text' => '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $lInfo->directory . '/images/' . $lInfo->image, $lInfo->name)];
