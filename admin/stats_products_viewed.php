...
/*
  $Id: stats_products_viewed.php,v 1.1 2006/03/27 09:05:33 mikhail Exp $
                            ...
                            <tr>
                                <td valign="top">
                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                        ...
                                        $rows = 0;
                                        $products_query_raw = 'select p.products_id, pd.products_name, pd.products_viewed, l.name from '
                                                              . TABLE_PRODUCTS
                                                              . ' p, '
                                                              . TABLE_PRODUCTS_DESCRIPTION
                                                              . ' pd, '
                                                              . TABLE_LANGUAGES
                                                              . ' l where p.products_id = pd.products_id and l.languages_id = pd.language_id order by pd.products_viewed DESC';
                                        ...
                                        while (false !== ($products = tep_db_fetch_array($products_query))) {
                                            ...
                                            <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"
                                                onclick="document.location.href='<?php echo tep_href_link(FILENAME_CATEGORIES, 'action=new_product_preview&read=only&pID=' . $products['products_id'] . '&origin=' . FILENAME_STATS_PRODUCTS_VIEWED . '?page=' . $_GET['page'], 'NONSSL'); ?>'">
                                                <td class="dataTableContent"><?php echo $rows; ?>.</td>
                                                <td class="dataTableContent"><?php echo '<a href="'
                                                                                        . tep_href_link(FILENAME_CATEGORIES, 'action=new_product_preview&read=only&pID=' . $products['products_id'] . '&origin=' . FILENAME_STATS_PRODUCTS_VIEWED . '?page=' . $_GET['page'], 'NONSSL')
                                                                                        . '">'
