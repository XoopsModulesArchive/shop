...
/*
  $Id: affiliate_payment.php,v 2.00 2004/10/12
...
while (false !== ($payments_status = tep_db_fetch_array($payments_status_query))) {
    $payments_statuses[] = [
...
switch ($_GET['action']) {
    case 'start_billing':
        ...
        // Start Billing:
        while (false !== ($affiliate_payment = tep_db_fetch_array($affiliate_payment_query))) {
        ...
        ";
            ...
            while (false !== ($affiliate_orders = tep_db_fetch_array($affiliate_orders_query))) {
                $orders_id .= $affiliate_orders['affiliate_orders_id'] . ',';
            }

            $orders_id = mb_substr($orders_id, 0, -1) . ')';
...
'affiliate_address_format_id' => $affiliate['address_format_id'],
            ...
            if (AFFILIATE_NOTIFY_AFTER_BILLING == 'true') {
                $check_status_query = tep_db_query(
                    'select af.affiliate_email_address, ap.affiliate_lastname, ap.affiliate_firstname, ap.affiliate_payment_status, ap.affiliate_payment_date, ap.affiliate_payment_date from '
                    . TABLE_AFFILIATE_PAYMENT
                    . ' ap, '
                    . TABLE_AFFILIATE
                    . " af where affiliate_payment_id  = '"
                    . $insert_id
                    . "' and af.affiliate_id = ap.affiliate_id "
                );
                ...
                $email = STORE_NAME
                                      ...
                                      . EMAIL_SEPARATOR
                                      . "\n"
                                      . EMAIL_TEXT_AFFILIATE_PAYMENT_NUMBER
                                      . ' '
                                      . $insert_id
                                      . "\n"
                                      . EMAIL_TEXT_INVOICE_URL
                                      . ' '
                                      . tep_catalog_href_link(FILENAME_CATALOG_AFFILIATE_PAYMENT_INFO, 'payment_id=' . $insert_id, 'SSL')
                                      . "\n"
                                      . EMAIL_TEXT_PAYMENT_BILLED
                                      . ' '
                                      . tep_date_long($check_status['affiliate_payment_date'])
                                      . "\n\n"
        ...
        tep_redirect(tep_href_link(FILENAME_AFFILIATE_PAYMENT, tep_get_all_get_params(['action']) . 'action=edit'));
        break;
    case 'update_payment':
        ...
        if ($check_status['affiliate_payment_status'] != $status) {
            ...
            if ('on' == $_POST['notify']) {
                $email = STORE_NAME
                         ...
                         . EMAIL_TEXT_INVOICE_URL
                         . ' '
                         . tep_catalog_href_link(FILENAME_CATALOG_AFFILIATE_PAYMENT_INFO, 'payment_id=' . $pID, 'SSL')
                         . "\n"
                         . EMAIL_TEXT_PAYMENT_BILLED
                         . ' '
                         . tep_date_long($check_status['affiliate_payment_date'])
            ...
            tep_db_query(
                'insert into '
                . TABLE_AFFILIATE_PAYMENT_STATUS_HISTORY
                . " (affiliate_payment_id, affiliate_new_value, affiliate_old_value, affiliate_date_added, affiliate_notified) values ('"
                . tep_db_input($pID)
                . "', '"
                . tep_db_input($status)
                . "', '"
    ...
    case 'deleteconfirm':
        ...
        tep_redirect(tep_href_link(FILENAME_AFFILIATE_PAYMENT, tep_get_all_get_params(['pID', 'action'])));
        break;
}

if (('edit' == $_GET['action']) && tep_not_null($_GET['pID'])) {
    $pID = tep_db_prepare_input($_GET['pID']);

    $payments_query = tep_db_query(
        'select p.*,  a.affiliate_payment_check, a.affiliate_payment_paypal, a.affiliate_payment_bank_name, a.affiliate_payment_bank_branch_number, a.affiliate_payment_bank_swift_code, a.affiliate_payment_bank_account_name, a.affiliate_payment_bank_account_number from '
        . TABLE_AFFILIATE_PAYMENT
        . ' p, '
        . TABLE_AFFILIATE
        . " a where affiliate_payment_id = '"
        . tep_db_input($pID)
    ...
    <tr>
        ...
        <td width="100%" valign="top">
            <table border="0" width="100%" cellspacing="0" cellpadding="2">
                <?php
                if (('edit' == $_GET['action']) && ($payments_exists)) {
                        ...
                        <td width="100%">
                            <table border="0" width="100%" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td class="pageHeading" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_PAYMENT, tep_get_all_get_params(['action'])) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                                                    ...
                                                    <table border="1" cellspacing="0" cellpadding="5">
                                                        <tr>
                                                            ...
                                                            if (AFFILIATE_USE_BANK == 'true') {
                                                                ?>
                                                                <td class="main" valign="top"><?php echo '<b>'
                                                                                                         . TEXT_AFFILIATE_PAYMENT_BANK_TRANSFER
                                                                                                         . '</b><br><br>'
                                                                                                         . TEXT_AFFILIATE_PAYMENT_BANK_NAME
                                                                                                         . ' '
                                                                                                         . $payments['affiliate_payment_bank_name']
                                                                                                         . '<br>'
                                                                                                         . TEXT_AFFILIATE_PAYMENT_BANK_BRANCH_NUMBER
                                                                                                         . ' '
                                                                                                         . $payments['affiliate_payment_bank_branch_number']
                                                                                                         . '<br>'
                                                                                                         . TEXT_AFFILIATE_PAYMENT_BANK_SWIFT_CODE
                                                                                                         . ' '
                                                                                                         . $payments['affiliate_payment_bank_swift_code']
                                                                                                         . '<br>'
                                                                                                         . TEXT_AFFILIATE_PAYMENT_BANK_ACCOUNT_NAME
                                                                                                         . ' '
                                                                                                         . $payments['affiliate_payment_bank_account_name']
                                                                                                         . '<br>'
                                                                                                         . TEXT_AFFILIATE_PAYMENT_BANK_ACCOUNT_NUMBER
                                                                                                         . ' '
                                                                                                         . $payments['affiliate_payment_bank_account_number']
                                                                                                         . '<br>'; ?></td>
                                                                <?php
                                                            }
                    ...
                    <tr>
                        <?php echo tep_draw_form('status', FILENAME_AFFILIATE_PAYMENT, tep_get_all_get_params(['action']) . 'action=update_payment'); ?>
                        <td>
                            <table border="0" cellspacing="0" cellpadding="2">
                                <tr>
                                    <td>
                    ...
                    <tr>
                        <td class="main">
                            <table border="1" cellspacing="0" cellpadding="5">
                                ...
                                <?php
                                $affiliate_history_query = tep_db_query(
                            'select affiliate_new_value, affiliate_old_value, affiliate_date_added, affiliate_notified from ' . TABLE_AFFILIATE_PAYMENT_STATUS_HISTORY . " where affiliate_payment_id = '" . tep_db_input($pID) . "' order by affiliate_status_history_id desc"
                        );

                    if (tep_db_num_rows($affiliate_history_query)) {
                        while (false !== ($affiliate_history = tep_db_fetch_array($affiliate_history_query))) {
                            echo '          <tr>'
                                             . "\n"
                                             . '            <td class="smallText">'
                                             . $payments_status_array[$affiliate_history['affiliate_new_value']]
                                             . '</td>'
                                             . "\n"
                                             . '            <td class="smallText">'
                                             . (tep_not_null(
                                                 $affiliate_history['affiliate_old_value']
                                             ) ? $payments_status_array[$affiliate_history['affiliate_old_value']] : '&nbsp;')
                                             . '</td>'
                                             . "\n"
                                             . '            <td class="smallText" align="center">'
                                             . tep_date_short($affiliate_history['affiliate_date_added'])
                                             . '</td>'
                                             . "\n"
                                             . '            <td class="smallText" align="center">';

                            if ('1' == $affiliate_history['affiliate_notified']) {
                    ...
                    <tr>
                        <td colspan="2" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_INVOICE, 'pID=' . $_GET['pID']) . '" TARGET="_blank">' . tep_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a> <a href="' . tep_href_link(
                        FILENAME_AFFILIATE_PAYMENT,
                                    ...
                                    <td align="right">
                                        <table border="0" width="100%" cellspacing="0" cellpadding="0">
                                            ...
                                            <tr><?php echo tep_draw_form('status', FILENAME_AFFILIATE_PAYMENT, '', 'get'); ?>
                                                <td class="smallText" align="right"><?php echo HEADING_TITLE_STATUS . ' ' . tep_draw_pull_down_menu('status', array_merge([['id' => '', 'text' => TEXT_ALL_PAYMENTS]], $payments_statuses), '', 'onChange="this.form.submit();"'); ?></td>
                                                </form></tr>
                                        </table>
                                    </td>
                                </tr>
                                    ...
                                    <td valign="top">
                                        <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                            ...
                                            if ($_GET['sID']) {
                                                ...
                                                $payments_query_raw = 'select p.* , s.affiliate_payment_status_name from '
                                                                      . TABLE_AFFILIATE_PAYMENT
                                                                      . ' p , '
                                                                      . TABLE_AFFILIATE_PAYMENT_STATUS
                                                                      . " s where p.affiliate_payment_id = '"
                                                                      . tep_db_input($sID)
                                                                      . "' and p.affiliate_payment_status = s.affiliate_payment_status_id and s.affiliate_language_id = '"
                                                                      . $languages_id
                                                ...
                                                $payments_query_raw = 'select p.* , s.affiliate_payment_status_name from '
                                                                      ...
                                                                      . ' p , '
                                                                      . TABLE_AFFILIATE_PAYMENT_STATUS
                                                                      . " s where s.affiliate_payment_status_id = '"
                                                                      . tep_db_input($status)
                                                                      . "' and p.affiliate_payment_status = s.affiliate_payment_status_id and s.affiliate_language_id = '"
                                                                      . $languages_id
                                                                      . "' order by p.affiliate_payment_id DESC";
                                                ...
                                                $payments_query_raw = 'select p.* , s.affiliate_payment_status_name from '
                                                                      ...
                                                                      . ' p , '
                                                                      . TABLE_AFFILIATE_PAYMENT_STATUS
                                                                      . " s where p.affiliate_payment_status = s.affiliate_payment_status_id and s.affiliate_language_id = '"
                                                                      . $languages_id
                                                                      . "' order by p.affiliate_payment_id DESC";
                                            }
                    ...
                    while (false !== ($payments = tep_db_fetch_array($payments_query))) {
                        ...
                        if ((is_object($pInfo)) && ($payments['affiliate_payment_id'] == $pInfo->affiliate_payment_id)) {
                            echo '              <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . tep_href_link(
                                FILENAME_AFFILIATE_PAYMENT,
                                tep_get_all_get_params(['pID', 'action']) . 'pID=' . $pInfo->affiliate_payment_id . '&action=edit'
                            ) . '\'">' . "\n";
                        } else {
                            echo '              <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(
                                ...
                                tep_get_all_get_params(
                                                                ['pID']
                                                            ) . 'pID=' . $payments['affiliate_payment_id']
                            ) . '\'">' . "\n";
                        } ?>
                                                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_PAYMENT, tep_get_all_get_params(['pID', 'action']) . 'pID=' . $pInfo->affiliate_payment_id . '&action=edit') . '">' . tep_image(
                            DIR_WS_ICONS . 'preview.gif',
                            ICON_PREVIEW
                        ) . '</a>&nbsp;' . $payments['affiliate_firstname'] . ' ' . $payments['affiliate_lastname']; ?></td>
                            ...
                            echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_PAYMENT, tep_get_all_get_params(['pID']) . 'pID=' . $payments['affiliate_payment_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
                        } ?>&nbsp;
                                                </td>
                                                </tr>
                                                <?php
                                                ...
                                                <td colspan="6">
                                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                                        <tr>
                                                            ...
                                                            <td class="smallText" align="right"><?php echo $payments_split->display_links($payments_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $_GET['page'], tep_get_all_get_params(['page', 'pID', 'action'])); ?></td>
                                                        </tr>
                                                    </table>
                                                </td>
                                            </tr>
                    ...
                    switch ($_GET['action']) {
                                        case 'delete':
                                            $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_DELETE_PAYMENT . '</b>'];

                                            $contents = ['form' => tep_draw_form('payment', FILENAME_AFFILIATE_PAYMENT, tep_get_all_get_params(['pID', 'action']) . 'pID=' . $pInfo->affiliate_payment_id . '&action=deleteconfirm')];
                                            $contents[] = ['text' => TEXT_INFO_DELETE_INTRO . '<br>'];
                                            $contents[] = [
                                                'align' => 'center',
'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(AFFILIATE_PAYMENT, tep_get_all_get_params(['pID', 'action']) . 'pID=' . $pInfo->affiliate_payment_id) . '">' . tep_image_button(
    'button_cancel.gif',
    IMAGE_CANCEL
) . '</a>',
                                            ];
                                        ...
                                        default:
                                            if (is_object($pInfo)) {
                                                $heading[] = ['text' => '<b>[' . $pInfo->affiliate_payment_id . ']&nbsp;&nbsp;' . tep_datetime_short($pInfo->affiliate_payment_date) . '</b>'];

                                                $contents[] = [
                                                    'align' => 'center',
'text' => '<a href="'
                                                               . tep_href_link(FILENAME_AFFILIATE_PAYMENT, tep_get_all_get_params(['pID', 'action']) . 'pID=' . $pInfo->affiliate_payment_id . '&action=edit')
                                                               . '">'
                                                               . tep_image_button('button_edit.gif', IMAGE_EDIT)
                                                               . '</a> <a href="'
                                                               . tep_href_link(FILENAME_AFFILIATE_PAYMENT, tep_get_all_get_params(['pID', 'action']) . 'pID=' . $pInfo->affiliate_payment_id . '&action=delete')
                                                               . '">'
                                                               . tep_image_button('button_delete.gif', IMAGE_DELETE)
                                                               . '</a>',
                                                ];

                                                $contents[] = ['align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_AFFILIATE_INVOICE, 'pID=' . $pInfo->affiliate_payment_id) . '" TARGET="_blank">' . tep_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a> '];
                                            }
                                            break;
                                    }
