...
/*
  $Id: email.php,v 1.1 2006/03/27 09:08:12 mikhail Exp $
...
'swf' => 'application/x-shockwave-flash',
    ...
    public function find_html_images($images_dir)
    {
        ...
        for ($i = 0, $iMax = count($images[1]); $i < $iMax; $i++) {
        ...
        if (tep_not_null($html_images)) {
            ...
            for ($i = 0, $iMax = count($html_images); $i < $iMax; $i++) {
                if ($image = $this->get_file($images_dir . $html_images[$i])) {
                    $content_type = $this->image_types[mb_substr($html_images[$i], mb_strrpos($html_images[$i], '.') + 1)];
...
'encoding' => $encoding,
    ...
    public function build_message($params = '')
    {
        ...
        switch (true) {
            case ((true === $text) && (false === $attachments)):
                /* HPDL PHP3 */ //          $message =& $this->add_text_part($null, $this->text);
                $message = $this->add_text_part($null, $this->text);
                break;
            case ((false === $text) && (true === $attachments) && (false === $html)):
                /* HPDL PHP3 */ //          $message =& $this->add_mixed_part();
                $message = $this->add_mixed_part();

                for ($i = 0, $iMax = count($this->attachments); $i < $iMax; $i++) {
                    $this->add_attachment_part($message, $this->attachments[$i]);
                }
                break;
            case ((true === $text) && (true === $attachments)):
                /* HPDL PHP3 */ //          $message =& $this->add_mixed_part();
                $message = $this->add_mixed_part();
                ...
                break;
            case ((true === $html) && (false === $attachments) && (false === $html_images)):
                ...
                break;
            case ((true === $html) && (false === $attachments) && (true === $html_images)):
                if (tep_not_null($this->html_text)) {
                    /* HPDL PHP3 */
                    ...
                    /* HPDL PHP3 */ //            $message =& $this->add_related_part($null);
                ...
                for ($i = 0, $iMax = count($this->html_images); $i < $iMax; $i++) {
                    $this->add_html_image_part($related, $this->html_images[$i]);
                }
                break;
            case ((true === $html) && (true === $attachments) && (false === $html_images)):
                /* HPDL PHP3 */ //          $message =& $this->add_mixed_part();
                $message = $this->add_mixed_part();
                if (tep_not_null($this->html_text)) {
                    /* HPDL PHP3 */
                ...
                break;
            case ((true === $html) && (true === $attachments) && (true === $html_images)):
                /* HPDL PHP3 */ //          $message =& $this->add_mixed_part();
                $message = $this->add_mixed_part();
    ...
    public function send($to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = '')
    {
        ...
        for ($i = 0, $iMax = count($headers); $i < $iMax; $i++) {
            if (is_array($headers[$i])) {
                for ($j = 0, $jMax = count($headers[$i]); $j < $jMax; $j++) {
                    if ('' != $headers[$i][$j]) {
