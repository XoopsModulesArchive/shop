<?php
/*
  $Id: order.php,v 1.1 2006/03/27 09:08:12 mikhail Exp $
...
class order
{
    ...
    public function query($order_id)
    {
        ...
        while (false !== ($totals = tep_db_fetch_array($totals_query))) {
            $this->totals[] = [
                'title' => $totals['title'],
                'text' => $totals['text'],
            ];
        ...
        $this->info = [
            ...
            'shipping_method' => ((':' == mb_substr($shipping_method['title'], -1)) ? mb_substr(strip_tags($shipping_method['title']), 0, -1) : strip_tags($shipping_method['title'])),
        ];
        ...
        while (false !== ($orders_products = tep_db_fetch_array($orders_products_query))) {
            ...
            $attributes_query = tep_db_query(
                'select products_options, products_options_values, options_values_price, price_prefix from ' . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " where orders_id = '" . (int)$order_id . "' and orders_products_id = '" . (int)$orders_products['orders_products_id'] . "'"
            );

            if (tep_db_num_rows($attributes_query)) {
                while (false !== ($attributes = tep_db_fetch_array($attributes_query))) {
            ...
            $this->info['tax_groups'][(string)($this->products[$index]['tax'])] = '1';
    ...
    public function cart()
    {
        ...
        $customer_address_query = tep_db_query(
            ...
            . TABLE_COUNTRIES
            . " co on (ab.entry_country_id = co.countries_id) where c.customers_id = '"
            . (int)$customer_id
            . "' and ab.customers_id = '"
            . (int)$customer_id
        ...
        $shipping_address_query = tep_db_query(
            'select ab.entry_firstname, ab.entry_lastname, ab.entry_company, ab.entry_street_address, ab.entry_suburb, ab.entry_postcode, ab.entry_city, ab.entry_zone_id, z.zone_name, ab.entry_country_id, c.countries_id, c.countries_name, c.countries_iso_code_2, c.countries_iso_code_3, c.address_format_id, ab.entry_state from '
            . TABLE_ADDRESS_BOOK
            . ' ab left join '
            . TABLE_ZONES
            . ' z on (ab.entry_zone_id = z.zone_id) left join '
            . TABLE_COUNTRIES
            . " c on (ab.entry_country_id = c.countries_id) where ab.customers_id = '"
            . (int)$customer_id
        ...
        $tax_address_query = tep_db_query(
            'select ab.entry_country_id, ab.entry_zone_id from '
            . TABLE_ADDRESS_BOOK
            . ' ab left join '
            . TABLE_ZONES
            . " z on (ab.entry_zone_id = z.zone_id) where ab.customers_id = '"
            . (int)$customer_id
            . "' and ab.address_book_id = '"
            . (int)('virtual' == $this->content_type ? $billto : $sendto)
            . "'"
        );
        ...
        for ($i = 0, $n = count($products); $i < $n; $i++) {
            ...
            if ($products[$i]['attributes']) {
                ...
                while (list($option, $value) = each($products[$i]['attributes'])) {
                    $attributes_query = tep_db_query(
                        'select popt.products_options_name, poval.products_options_values_name, pa.options_values_price, pa.price_prefix from '
                        . TABLE_PRODUCTS_OPTIONS
                        . ' popt, '
                        . TABLE_PRODUCTS_OPTIONS_VALUES
                        . ' poval, '
                        . TABLE_PRODUCTS_ATTRIBUTES
                        . " pa where pa.products_id = '"
                        . (int)$products[$i]['id']
                        . "' and pa.options_id = '"
                        . (int)$option
            ...
            if (DISPLAY_PRICE_WITH_TAX == 'true') {
                ...
                if (isset($this->info['tax_groups'][(string)$products_tax_description])) {
                    $this->info['tax_groups'][(string)$products_tax_description] += $shown_price - ($shown_price / (($products_tax < 10) ? '1.0' . str_replace('.', '', $products_tax) : '1.' . str_replace('.', '', $products_tax)));
                } else {
                    $this->info['tax_groups'][(string)$products_tax_description] = $shown_price - ($shown_price / (($products_tax < 10) ? '1.0' . str_replace('.', '', $products_tax) : '1.' . str_replace('.', '', $products_tax)));
                }
            } else {
                ...
                if (isset($this->info['tax_groups'][(string)$products_tax_description])) {
                    $this->info['tax_groups'][(string)$products_tax_description] += ($products_tax / 100) * $shown_price;
                } else {
                    $this->info['tax_groups'][(string)$products_tax_description] = ($products_tax / 100) * $shown_price;
                }
            }
