<?php
/*
  $Id: general.php,v 1.1 2006/03/27 09:08:28 mikhail Exp $
...
function tep_get_countries($countries_id = '', $with_iso_codes = false)
{
    ...
    if (tep_not_null($countries_id)) {
        ...
        while (false !== ($countries_values = tep_db_fetch_array($countries))) {
            $countries_array[] = [
...
function tep_get_tax_rate($class_id, $country_id = -1, $zone_id = -1)
{
    ...
    $tax_query = tep_db_query(
        "select sum(tax_rate) as tax_rate from "
        . TABLE_TAX_RATES
        . " tr left join "
    ...
    if (tep_db_num_rows($tax_query)) {
        $tax_multiplier = 1.0;
        while (false !== ($tax = tep_db_fetch_array($tax_query))) {
...
function tep_count_products_in_category($category_id, $include_inactive = false)
{
    ...
    if (tep_db_num_rows($child_categories_query)) {
        while (false !== ($child_categories = tep_db_fetch_array($child_categories_query))) {
...
function tep_address_label($customers_id, $address_id = 1, $html = false, $boln = '', $eoln = "\n")
{
    $address_query = tep_db_query(
        "select entry_firstname as firstname, entry_lastname as lastname, entry_company as company, entry_street_address as street_address, entry_suburb as suburb, entry_city as city, entry_postcode as postcode, entry_state as state, entry_zone_id as zone_id, entry_country_id as country_id from "
        . TABLE_ADDRESS_BOOK
        . " where customers_id = '"
        . (int)$customers_id
        . "' and address_book_id = '"
...
function tep_get_categories($categories_array = '', $parent_id = '0', $indent = '')
{
    ...
    $categories_query = tep_db_query(
        "select c.categories_id, cd.categories_name from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where parent_id = '" . (int)$parent_id . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' order by sort_order, cd.categories_name"
    );
...
function tep_get_subcategories(&$subcategories_array, $parent_id = 0)
{
    ...
    while (false !== ($subcategories = tep_db_fetch_array($subcategories_query))) {
...
function tep_parse_search_string($search_str = '', &$objects)
{
    ...
    for ($k = 0, $kMax = count($pieces); $k < $kMax; $k++) {
        ...
        if ((substr($pieces[$k], -1) != '"') && (substr($pieces[$k], 0, 1) != '"')) {
            ...
            for ($j = 0, $jMax = count($post_objects); $j < $jMax; $j++) {
...
function tep_create_sort_heading($sortby, $colnum, $heading)
{
    ...
    if ($sortby) {
        $sort_prefix = '<a href="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(['page', 'info', 'sort']) . 'page=1&sort=' . $colnum . ($sortby == $colnum . 'a' ? 'd' : 'a')) . '" title="' . tep_output_string(
                TEXT_SORT_PRODUCTS . ($sortby == $colnum . 'd' || substr($sortby, 0, 1) != $colnum ? TEXT_ASCENDINGLY : TEXT_DESCENDINGLY) . TEXT_BY . $heading
...
function tep_get_parent_categories(&$categories, $categories_id)
{
    ...
    while (false !== ($parent_categories = tep_db_fetch_array($parent_categories_query))) {
...
{
    $uprid = $prid;
    if ((is_array($params)) && (false === strpos($prid, '{'))) {
...
function tep_mail($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address)
{
    ...
    $message = new email(['X-Mailer: osCommerce Mailer']);
...
?>
SESSION['customer_first_name']) && tep_isset(\<?php
/*
  $Id: general.php,v 1.1 2006/03/27 09:08:28 mikhail Exp $
...
?>
SESSION['customer_id']) === false) || ($id != $customer_id)) {
            return 0;
        }
    }
