<?php
/*
  $Id: new_products.php,v 1.1 2006/03/27 09:13:58 mikhail Exp $
...
<?php
$info_box_contents   = [];
$info_box_contents[] = ['text' => sprintf(TABLE_HEADING_NEW_PRODUCTS, strftime('%B'))];
...
while (false !== ($new_products = tep_db_fetch_array($new_products_query))) {
    ...
    $info_box_contents[$row][$col] = [
        ...
        'text'   => '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $new_products['products_id']) . '">' . tep_image(DIR_WS_IMAGES . $new_products['products_image'], $new_products['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a><br><a href="' . tep_href_link(
